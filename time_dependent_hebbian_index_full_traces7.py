...
rt = [x[:-1] for x in RT]
hi = [x[1:] for x in hib]
_,_,p = pf.mean_bin_plot(np.concatenate(rt),np.concatenate(hi),5,1,1,'k');
plt.xlabel('Reward time on previous trial (s)')
plt.ylabel('Hebbian index at pre')
print('p = ' + str(p))
#%%
