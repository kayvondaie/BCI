...
import suite2p
import os
os.chdir('G:/My Drive/Python Scripts/BCI_analysis/')
import re
...
folder = [r'//allen/aind/scratch/BCI/2p-raw/BCINM_006/011724/']
...
savefolders = dict()
savefolders[0,0] = 'BCI_green';
savefolders[1,0] = 'spont_green';
savefolders[0,1] = 'BCI_red';
savefolders[1,1] = 'spont_red';
#savefolders[2] = 'spont';
...
print(bases)
ind = input('pick indices of bases for BCI, spont in that order')
...
for chan in range(2):
    for ei in range(0,len(ind)):
        ...
        if 'old_folder' in locals():
            ...
            ops['save_folder'] = 'suite2p_' + savefolders[ei,chan]    
        ...
        np.save(folder + 'suite2p_' + savefolders[ei,chan] + r'/plane0/siHeader.npy',siHeader)
    
        import data_dict_create_module as ddc
        if chan == 0:
            green_data = ddc.main(folder)
        else:
            red_data = ddc.main(folder)
#%%
