...
im = plt.imshow(np.nanmean(np.stack(BETA),0), cmap='coolwarm', aspect='auto')
plt.colorbar(im, fraction=0.046, pad=0.04, label='Standardized β')
plt.xticks(range(len(preds)), preds)
plt.yticks(range(len(epochs)), epochs)    
#$$
...
# ================================================================
# Pooled regression across sessions (with Ridge option)
# ================================================================
import numpy as np
import matplotlib.pyplot as plt
from numpy.linalg import lstsq, inv
...
# ---------- config ----------
use_ridge = False   # <-- set True to use RidgeCV instead of OLS
ridge_alphas = np.logspace(-3, 3, 20)  # search space for RidgeCV
...
def fit_ridge_with_pvals(X, y, alpha=1.0, n_boot=500, random_state=0):
    """
    Ridge regression with bootstrap-based p-values.
    
    Parameters
    ----------
    X : array, shape (n_samples, n_features)
        Design matrix (include intercept column if you want it).
    y : array, shape (n_samples,)
        Response vector.
    alpha : float
        Ridge penalty strength (λ).
    n_boot : int
        Number of bootstrap iterations for empirical null.
    random_state : int
    ...
    -------
    dict with:
        'coef' : ridge coefficients (n_features,)
        'yhat' : fitted values
        'R2'   : coefficient of determination
        'pvals': bootstrap p-values for each coefficient
    """
    ...
    null_coefs = np.zeros((n_boot, X.shape[1]))
    for b in range(n_boot):
        # resample trials WITH replacement
        idx = rng.integers(0, len(y), len(y))
        Xb, yb = X[idx], y[idx]

        ridge_b = Ridge(alpha=alpha, fit_intercept=False)
        ridge_b.fit(Xb, yb)
        null_coefs[b, :] = ridge_b.coef_

    # two-sided empirical p-values
    pvals = np.mean(np.abs(null_coefs) >= np.abs(coef), axis=0)

    return dict(coef=coef, yhat=yhat, R2=R2, pvals=pvals)


def fit_std(X, y):
    """OLS with standardized predictors and response."""
    y = np.ravel(y)
    m = np.isfinite(y) & np.all(np.isfinite(X), 1)
    X, y = X[m], y[m]

    if use_ridge:
        # --- RidgeCV with standardized predictors and response ---
        ...
        # pad with dummy p-values (not defined for Ridge)
...
rpe_all, rt_all, hit_all, hit_rpe_all, miss_rpe_all = [], [], [], [], []
pre_all, early_all, late_all, rew_all = [], [], [], []

for pre_s, early_s, late_s, rew_s, rpe_s, rt_s, hit_s, hit_rpe_s, miss_rpe_s in zip(
        pre_list, early_list, late_list, rew_list,
        rpe_list, rt_list, hit_list, hit_rpe_list, miss_rpe_list):
...
for i in range(len(epochs)):
    for j in range(len(preds)):
        ...
        if s:
            plt.text(j, i, s, ha='center', va='center', color='k', fontsize=11)
plt.title(f'Coefficient heatmap ({"RidgeCV" if use_ridge else "OLS"})')
plt.tight_layout()
plt.show()
