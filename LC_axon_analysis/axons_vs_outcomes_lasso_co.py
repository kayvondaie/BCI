# -*- coding: utf-8 -*-
"""
Code Ocean / portable version of axons_vs_outcomes_lasso.py.

Reads the PACKAGED NM data (F:\nm_data_2026 locally, or a Code Ocean data-asset
mount) instead of the //allen share, so the axon-vs-outcomes Lasso analysis can be
reproduced off-VPN. Use it to VALIDATE the packaged F: bundle before uploading to CO.

Only the data I/O, imports, and figure-saving differ from the original; the compute
and regression are unchanged.

------------------------------------------------------------------------------
SET DATA_ROOT for your environment:
  - Local validation:  r'F:\nm_data_2026'
  - Code Ocean:        '/root/capsule/data/<nm_asset_name>'

Expected layout (produced by copy_sessions_nm.py):
  {DATA_ROOT}/{mouse}/{session}/data_main_{mouse}_{session}_BCI.npy
  {DATA_ROOT}/{mouse}/{session}/suite2p_BCI/plane0/ops.npy
------------------------------------------------------------------------------
"""
import os, sys

# ---- portable path setup (repo root + LC_axon_analysis on sys.path) ----
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
_REPO = os.path.dirname(_THIS_DIR) if os.path.basename(_THIS_DIR) == 'LC_axon_analysis' else _THIS_DIR
for _p in (_REPO, os.path.join(_REPO, 'LC_axon_analysis')):
...
import plotting_functions as pf          # explicit: original relied on Spyder namespace
from scipy.signal import medfilt, correlate
from scipy.stats import pearsonr
from BCI_data_helpers import *           # compute_rpe_standard, zscore helpers, etc.
from axon_helper_module import *         # reward_aligned_responses, get_axon_data_dict

# ============================ CONFIG ============================
DATA_ROOT = r'F:\nm_data_2026'           # <-- switch to '/root/capsule/data/<asset>' on CO
MICE = ["BCINM_017", "BCINM_023", "BCINM_024", "BCINM_027", "BCINM_031", "BCINM_034"]
RESULTS_DIR = os.path.join(_THIS_DIR, 'results')
os.makedirs(RESULTS_DIR, exist_ok=True)
# ===============================================================

# Point session enumeration at the packaged data (works for F: and CO mounts)
...
def _resolve_folder(mouse, session):
    """Return the folder holding data_main (pophys/ first, then session root)."""
    base = os.path.join(DATA_ROOT, mouse, session)
    fname = f"data_main_{mouse}_{session}_BCI.npy"
    for cand in (os.path.join(base, 'pophys'), base):
        if os.path.isfile(os.path.join(cand, fname)):
            return cand + os.sep
    return base + os.sep  # let the load error surface if truly missing


def _load_data_main(path):
    """Load data_main, unwrapping a 0-d object array back to a dict if needed.
    (A dict round-tripped through np.save becomes array(dict, dtype=object).)"""
    d = np.load(path, allow_pickle=True)
    if isinstance(d, np.ndarray) and d.dtype == object and d.ndim == 0:
        d = d.item()
    return d
...
for i in inds:
    try:
        ...
        # Load data from the packaged root (F: or CO mount)
        folder = _resolve_folder(mouse, session)
        main_npy_filename = f"data_main_{mouse}_{session}_BCI.npy"
        data = _load_data_main(os.path.join(folder, main_npy_filename))
...
print(f"\nSessions successfully loaded: {len(pre_list)}/{len(inds)}")
if len(pre_list) == 0:
    raise SystemExit("No sessions loaded — check DATA_ROOT and the packaged layout.")
...
for model_name, (coef_mat, p_mat, R2_list) in results.items():
    ...
    fig.savefig(os.path.join(RESULTS_DIR, f'axon_lasso_coef_{model_name}.png'), dpi=200, bbox_inches='tight')
    fig.savefig(os.path.join(RESULTS_DIR, f'axon_lasso_coef_{model_name}.svg'), bbox_inches='tight')
    plt.show()
...
plt.tight_layout()
fig.savefig(os.path.join(RESULTS_DIR, 'axon_lasso_calibration.png'), dpi=200, bbox_inches='tight')
fig.savefig(os.path.join(RESULTS_DIR, 'axon_lasso_calibration.svg'), bbox_inches='tight')
plt.show()

print(f"\nDone. R2 by epoch: " + ", ".join(f"{ep}={R2_dict[ep]:.3f}" for ep in Y.keys()))
