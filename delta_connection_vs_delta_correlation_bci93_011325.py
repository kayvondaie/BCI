...
INDS = []
A =  np.zeros(favg.shape[1],)
for ci in range(favg.shape[1]):
    ind = np.where((stimDist[ci,:] > 30)&(stimDist[ci,:] < 300))[0]
    A[ci] = np.nanmean(amp[ci,ind])
    INDS.append(ind)

# i_cells = np.where(A > .2)[0]
# X = [X[i] for i in i_cells if i < len(X)]  # Ensure index is valid
# Y = [Y[i] for i in i_cells if i < len(Y)]
# Yo = [Yo[i] for i in i_cells if i < len(Yo)]
X = np.concatenate(X)
...
#%%
hi = np.zeros(favg.shape[1])
nearest_idx = np.argmin(stimDist, axis=0)  
num_targets = stimDist.shape[1]
