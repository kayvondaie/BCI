...
for mi in range(len(mice)):
    ...
    pairwise_mode = 'dot_prod'  #dot_prod, noise_corr, dot_prod_no_mean, dot_prod_z, pre_only, post_only
    ...
    #for sii in range(si,si+1):
        try:
            ...
            Xstep_T, Xrew_T, Xts_T, Xpre_T, Y_T, used_bins = build_pairwise_XY_per_bin(
            ...
            # behavior_features must match the bins you kept in XX*
            # easiest: build it from the same kept bin indices you used to append XX*
            ...
            # z-score columns 0:4 across bins (leave last column = ones alone)
            ...
            if fitting == 1:
                for train_idx, test_idx in split_iter:
                    ...
                    if fit_type == 'pinv':
                        ...
                        lasso_alphas_fold = np.logspace(-10,-4, 10)
                        ...
                        beta = lasso_model.named_steps['lassocv'].coef_
            ...
            if plotting == 1:
                ...
                sns.heatmap(beta_reshaped*1, annot=True, xticklabels=[
                    'rt', 'rt_rpe', 'hit', 'hit_rpe','2-factor'
...
# ----------------------------
# Z-score Y using TRAIN stats only
# ----------------------------
...
# ----------------------------
# FAST model (fixed alpha)
# ----------------------------
alpha_fast = 10**(-3.45)  # try 1e-4 if all zeros; 1e-2 if too dense
...
print("nnz:", (beta != 0).sum(), "/", beta.size, " alpha:", alpha_fast)
print(f"Train r={r_tr:.3f} (p={p_tr:.2e}) | Test r={r_te:.3f} (p={p_te:.2e})")

# ----------------------------
# Reshape beta for heatmap (4 epochs × n_features)
# ----------------------------
n_features = beta.size // 4
beta_reshaped = beta.reshape(4, n_features)
