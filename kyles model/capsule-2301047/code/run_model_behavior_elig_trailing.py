#%% ============================================================================
# Model HI vs behavior x eligibility 4x4, eligibility computed with a TRAILING-20
# trial baseline (causal running mean over the most recent <=20 trials) instead of
# the fixed first-20 baseline. This tracks/removes slow drift (killing the session-
# ramp confound) while keeping ~20-trial-scale structure -- the user's real data
# convention. Per-trial activity; HI(div)=slope(whole-session dW, per-div elig);
# corr HI(div) with behavior across divisions. 15 seeds.
import os, sys
import numpy as np
...
def trailing_baseline(A, w):
    ...
    bl = np.empty_like(A)
    for t in range(n):
        bl[t] = A[max(0, t - w + 1):t + 1].mean(0)      # most recent <=w trials, causal
    return bl
...
np.save(os.path.join(OUT, 'model_behavior_elig_trailing.npy'), allres, allow_pickle=True)
...
ax.set_title('MODEL, trailing-20 baseline elig (per-trial), n=15')
cb = fig.colorbar(im, ax=ax, shrink=0.8, pad=0.04); cb.set_label('mean Spearman $\\rho$')
for ext in ('png', 'svg'):
    fig.savefig(os.path.join(OUT, 'talk_fig_model_4x4_trailing.' + ext), dpi=200, bbox_inches='tight')

print("\nMODEL (trailing-20 elig) HI vs behavior x form (mean rho):")
for bi, b in enumerate(BEH):
