#%% ============================================================================
# Validation of the eligibility-fingerprint analysis across 15 seeds.
# Per seed: regress dW ~ [I, S_post, S_pre, M] with the TRUE RPE and with a
# shuffle null (mean of N_SHUF timestep-shuffles). Aggregate.
# Expect: S_post(true) reliably > 0, S_post(shuffle) ~ 0, S_pre ~ 0 throughout
#         -> the analysis identifies POST-deviation and it rides on RPE timing.
import os, sys
import numpy as np
...
def run_seed(seed):
    ...
    def betas(rpe_raw):
        ...
        X = np.column_stack([zc(I[off]), zc(np.outer(q, m)[off]),
                             zc(np.outer(m, q)[off]), zc(M[off])])
        b, *_ = np.linalg.lstsq(X, y, rcond=None)
        return b

    b_true = betas(expand(total_rpes))
    rng = np.random.default_rng(seed)
    b_shuf = np.mean([betas(expand(total_rpes[rng.permutation(L)])) for _ in range(N_SHUF)], axis=0)
...
for s in SEEDS:
    try:
        ...
        print("seed {:2d}: S_post true={:+.3f} shuf={:+.3f} | S_pre true={:+.3f} shuf={:+.3f}".format(
            s, bt[1], bs[1], bt[2], bs[2]))
    except Exception as e:
        print("seed {} FAILED: {}".format(s, e))
BT = np.array(BT); BS = np.array(BS)
np.save(os.path.join(OUT, 'model_form_identification_multiseed.npy'), {'true': BT, 'shuf': BS}, allow_pickle=True)
...
print("\nAcross {} seeds (standardized beta, mean +/- sem):".format(len(BT)))
print("  {:10s} {:>18s} {:>18s}".format('block', 'true RPE', 'shuffled'))
...
d = BT[:, 1] - BS[:, 1]
print("\n  S_post(true) - S_post(shuffle): {:+.3f} +/- {:.3f}  {}  (paired)".format(
    d.mean(), d.std() / np.sqrt(len(d)), star(d)))

fig = plt.figure(figsize=(4.6, 3.0))
fw, fh = fig.get_size_inches()
ax = fig.add_axes([1.0 / fw, 0.95 / fh, 3.3 / fw, 1.8 / fh])
x = np.arange(4)
for j, (B, c, lab) in enumerate([(BT, '#1baf7a', 'true RPE'), (BS, '#b0b0b0', 'shuffled RPE')]):
...
ax.set_title('Eligibility fingerprint across 15 seeds', fontsize=8)
ax.legend(fontsize=7, frameon=False, loc='best')
for ext in ('png', 'svg'):
    fig.savefig(os.path.join(OUT, 'talk_fig_form_identification_multiseed.' + ext), dpi=200, bbox_inches='tight')
print("\nSaved to", OUT)
