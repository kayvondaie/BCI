#%% ============================================================================
# Easier-task variant: lower the spout-move threshold so the task is easier from
# the start of training -> higher, narrower hit rate. Then recompute the same
# hit-rate probe and the 4x4 (behavior x eligibility) matrix, so we can see how a
# narrower hit-rate range impacts the results.
#
# Knob: task_params['dyn_threshold_perc'] (default 0.7). Threshold is set at this
# percentile of stabilization BCI activity (kyle_pipeline line ~1384). Lower =
# easier. Set THRESH_PERC below; everything else is identical to the baseline
# run_model_behavior_elig_matrix.py / run_model_hit_rate.py.
import os, sys
import numpy as np
...
THRESH_PERC = 0.5          # <-- difficulty knob (baseline 0.7). Lower = easier.
SEEDS = list(range(15))
...
def run_seed(seed):
    ...
    params[0]['dyn_threshold_perc'] = THRESH_PERC        # <-- make task easier
    ...
    # hit rate
    hits = np.asarray(kp.get_rpe_estimates(task_hist)['hits'], float)
    third = len(hits) // 3
    hr = (np.mean(hits), np.mean(hits[:third]), np.mean(hits[-third:]))
    # 4x4 matrix
...
for s in SEEDS:
    try:
        hr, res = run_seed(s)
        hrs.append(hr); allres.append(res)
        print("seed {:2d}: hit rate = {:.3f}  (early {:.3f} -> late {:.3f})".format(s, hr[0], hr[1], hr[2]))
    except Exception as e:
        print("seed {} FAILED: {}".format(s, e))

np.save(os.path.join(OUT, 'model_behavior_elig_matrix_easy.npy'), allres, allow_pickle=True)

hr = np.array(hrs)
print("\nEASIER TASK (dyn_threshold_perc={}), MODEL hit rate (n={} seeds):".format(THRESH_PERC, len(hr)))
print("  overall = {:.3f} +/- {:.3f} sem   (baseline 0.7 was 0.771 +/- 0.046)".format(
    np.mean(hr[:, 0]), np.std(hr[:, 0]) / np.sqrt(len(hr))))
print("  early third = {:.3f}   late third = {:.3f}".format(np.mean(hr[:, 1]), np.mean(hr[:, 2])))
print("  range across seeds = [{:.3f}, {:.3f}]".format(np.min(hr[:, 0]), np.max(hr[:, 0])))
...
ax.set_title('MODEL, easier task (thr={}, n=15)'.format(THRESH_PERC))
cb = fig.colorbar(im, ax=ax, shrink=0.8, pad=0.04); cb.set_label('mean Spearman $\\rho$')
for ext in ('png', 'svg'):
    fig.savefig(os.path.join(OUT, 'talk_fig_model_4x4_easy.' + ext), dpi=200, bbox_inches='tight')

print("\nMODEL 4x4 (easier task), mean rho:")
print("  {:16s}".format('behavior') + ''.join('{:>16s}'.format(l.replace('$', '').replace('\\', '')) for l in ELIG_LABEL))
for bi, beh in enumerate(BEH):
    print("  {:16s}".format(BEH_LABEL[bi].split(' ')[0]) + ''.join('{:+16.3f}'.format(mat[bi, fi]) for fi in range(len(ELIG))))
print("\nSaved to", OUT)
