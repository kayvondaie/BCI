...
data['reward_time'], data['step_time'], data['trial_start']= ddc.create_zaber_info(folder,base)
steps = data['step_time']
vel = np.zeros(len(t_si),)
for i in range(len(steps)-99):
    ind = np.where(t_si>steps[i])[0][0]
    vel[ind] = 1
plt.subplot(3,1,3)
plt.plot(t_si[xlim[0]:xlim[1]],vel[xlim[0]:xlim[1]])
plt.show()
...
titles.append('Early')
titles.append('Late')
y = np.nanmean(np.nanmean(sta[stm_cls,:,early[0]:early[1]],axis = 2),axis = 0)
y = medfilt(y,kernel_size=11)
y_err = np.nanstd(np.nanmean(sta[stm_cls,:,early[0]:early[1]],axis = 0),axis = 1)/np.sqrt(early[1]-early[0])/2
axes[0].fill_between(tsta, y - y_err, y + y_err, color='k', alpha=0.2)
axes[0].plot(tsta,y,'k',linewidth = .2)
y = np.nanmean(np.nanmean(sta[stm_cls,:,late[0]:late[1]],axis = 2),axis = 0)
y = medfilt(y,kernel_size=11)
y_err = np.nanstd(np.nanmean(sta[stm_cls,:,late[0]:late[1]],axis = 0),axis = 1)/np.sqrt(late[1]-late[0])/2
axes[0].fill_between(tsta, y - y_err, y + y_err, color='m', alpha=0.2)
axes[0].plot(tsta,y,'m',linewidth = .2)
...
y = np.nanmean(sta[cns,:,early[0]:early[1]],axis = 1)
y = medfilt(y,kernel_size=11)
y_err = np.nanstd(sta[cns,:,early[0]:early[1]],axis = 1)/np.sqrt(early[1]-early[0])/2
axes[1].plot(tsta,y,'k',linewidth = .2)
axes[1].fill_between(tsta, y - y_err, y + y_err, color='k', alpha=0.2)
y = np.nanmean(sta[cns,:,late[0]:late[1]],axis = 1)
y = medfilt(y,kernel_size=11)
y_err = np.nanstd(sta[cns,:,late[0]:late[1]],axis = 1)/np.sqrt(late[1]-late[0])/2
axes[1].plot(tsta,y,'m',linewidth = .2)
axes[1].fill_between(tsta, y - y_err, y + y_err, color='m', alpha=0.2)
axes[1].set_title('CN')
...
avg = np.nanmean(sta[:,10:70,late[0]:late[1]],axis = 2)-np.nanmean(sta[:,10:70,early[0]:early[1]],axis = 2)
...
fldr = r'C:\Users\kayvon.daie\OneDrive - Allen Institute\written\NIH RO1 2023\figures\panels\'
plt.savefig(fldr+data['mouse']+"_"+data['session']+r'BCI_gated_summary2.pdf', format='pdf')
plt.show()
