...
# CC modes (2x2 of raw / mean-subtracted on pre & post; dev2 early-trial baseline):
#   'dot_prod_lag'   — sum_t pre(t)          * post(t+lag)           = r_pre  r_post
#   'dev2_lag'       — sum_t pre(t)          * (post(t+lag) - bl_post)= r_pre  dr_post
#   'post_dpre_lag'  — sum_t (pre(t)-bl_pre) * post(t+lag)           = dr_pre r_post
#   'dpost_dpre_lag' — sum_t (pre(t)-bl_pre) * (post(t+lag) - bl_post)= dr_pre dr_post
CC_MODES = ['dot_prod_lag', 'dev2_lag', 'post_dpre_lag', 'dpost_dpre_lag']
...
for mi in range(len(mice)):
    ...
    for sii in range(len(session_inds)):
        try:
            ...
            # Baseline post/pre mean per epoch (early-trial dev2 baseline)
            ...
            # ---- Eligibility-form similarity (pre epoch, full session) ----
            # Full-session per-pair eligibility for each of the 4 forms, then the
            # 4x4 correlation across pairs. Order: r_pre r_post, r_pre dr_post,
            # dr_pre dr_post, dr_pre r_post.  (Same forms as the CC modes.)
            _ep = 'pre'
            _rpre_all = cl_weights @ epoch_pre_act[_ep]                         # (n_pairs, trl)
            _rpost_all = epoch_post_act[_ep][all_nt, :]                         # (n_pairs, trl)
            _dpost_all = _rpost_all - baseline_post_mean_ep[_ep][all_nt, np.newaxis]
            _dpre_all = _rpre_all - (cl_weights @ baseline_pre_mean_ep[_ep])[:, np.newaxis]
            _Emat = np.vstack([
                np.nansum(_rpre_all * _rpost_all, axis=1),   # r_pre  r_post
                np.nansum(_rpre_all * _dpost_all, axis=1),   # r_pre  dr_post
                np.nansum(_dpre_all * _dpost_all, axis=1),   # dr_pre dr_post
                np.nansum(_dpre_all * _rpost_all, axis=1),   # dr_pre r_post
            ])
            elig_corr4 = np.corrcoef(_Emat)                                    # (4, 4)

            # ---- Compute CC per window per epoch (same structure as v2) ----
            raw_cc = np.full((n_wins, n_pairs, n_epochs), np.nan)          # r_pre  r_post
            dev2_cc = np.full((n_wins, n_pairs, n_epochs), np.nan)         # r_pre  dr_post
            post_dpre_cc = np.full((n_wins, n_pairs, n_epochs), np.nan)    # dr_pre r_post
            dpost_dpre_cc = np.full((n_wins, n_pairs, n_epochs), np.nan)   # dr_pre dr_post
            ...
            for wi, ws in enumerate(win_starts):
                ...
                for ei, ep in enumerate(EPOCH_ORDER):
                    ...
                    # deviations (early-trial dev2 baseline), pre projected through cl_weights
                    post_dev = post_act - baseline_post_mean_ep[ep][all_nt, np.newaxis]
                    pre_dev = pre_act - (cl_weights @ baseline_pre_mean_ep[ep])[:, np.newaxis]

                    # dev2: pre(t) * (post(t+lag) - mean_post_baseline)  = r_pre dr_post
                    dev2_cc[wi, :, ei] = np.sum(pre_act * post_dev, axis=1)
                    # post_dpre: (pre - bl_pre) * post                    = dr_pre r_post
                    post_dpre_cc[wi, :, ei] = np.sum(pre_dev * post_act, axis=1)
                    # dpost_dpre: (pre - bl_pre) * (post - bl_post)       = dr_pre dr_post
                    dpost_dpre_cc[wi, :, ei] = np.sum(pre_dev * post_dev, axis=1)
            ...
            for mode in CC_MODES:
                ...
                for ei, ep in enumerate(EPOCH_ORDER):
                    if mode == 'dot_prod_lag':
                        ...
                        cc_all = dev2_cc[:, :, ei]
                    elif mode == 'post_dpre_lag':
                        cc_all = post_dpre_cc[:, :, ei]
                    elif mode == 'dpost_dpre_lag':
                        cc_all = dpost_dpre_cc[:, :, ei]
                ...
                result = {
                    ...
                    'elig_corr4': elig_corr4,   # 4x4 eligibility-form similarity (pre epoch)
                }
...
# CELL 4: Save
# ============================================================================
np.save(os.path.join(RESULTS_DIR, 'sliding_window_four_elig.npy'),
...
# ============================================================================
all_results = np.load(
    os.path.join(RESULTS_DIR, 'sliding_window_four_elig.npy'),
...
#%% ============================================================================
# CELL 5b: Mean hit rate across sessions (data) -- to compare with the model
# ============================================================================
# 'hit_rate' per session = fraction of rewarded (hit) trials = np.nanmean(hit).
_hr = np.array([s['hit_rate'] for s in all_results[CC_MODES[0]]], dtype=float)
_hr = _hr[np.isfinite(_hr)]
print(f"\nDATA mean hit rate (fraction of rewarded trials), n={len(_hr)} sessions:")
print(f"  mean   = {np.mean(_hr):.3f} +/- {np.std(_hr) / np.sqrt(len(_hr)):.3f} sem")
print(f"  median = {np.median(_hr):.3f}   range = [{np.min(_hr):.3f}, {np.max(_hr):.3f}]")

#%% ============================================================================
# CELL 5c: Eligibility-form similarity 4x4 (data) -- mean of per-session corr
# ============================================================================
# elig_corr4 per session = 4x4 Pearson corr, across pairs, of the per-pair
# full-session eligibility for the 4 forms (pre epoch):
#   r_pre r_post, r_pre dr_post, dr_pre dr_post, dr_pre r_post.
# Per-session then averaged (avoids mixing session-level scales).
_ELIG_LAB = [r'$r_{pre}r_{post}$', r'$r_{pre}\Delta r_{post}$',
             r'$\Delta r_{pre}\Delta r_{post}$', r'$\Delta r_{pre}r_{post}$']
_cmats = np.array([s['elig_corr4'] for s in all_results[CC_MODES[0]]
                   if ('elig_corr4' in s and np.all(np.isfinite(s['elig_corr4'])))])
_Cmean = np.nanmean(_cmats, axis=0)
print(f"\nDATA 4x4 eligibility correlation (mean over {len(_cmats)} sessions):")
print("  order: r_pre r_post, r_pre dr_post, dr_pre dr_post, dr_pre r_post")
print(np.round(_Cmean, 3))

_fig, _ax = plt.subplots(figsize=(4.4, 4.0))
_im = _ax.imshow(_Cmean, cmap='coolwarm', vmin=-1, vmax=1, aspect='auto')
for _i in range(4):
    for _j in range(4):
        _ax.text(_j, _i, f'{_Cmean[_i, _j]:.2f}', ha='center', va='center',
                 color='white' if abs(_Cmean[_i, _j]) > 0.6 else 'k')
_ax.set_xticks(range(4)); _ax.set_xticklabels(_ELIG_LAB, rotation=40, ha='right')
_ax.set_yticks(range(4)); _ax.set_yticklabels(_ELIG_LAB)
_ax.set_title(f'DATA: eligibility-form similarity\n(mean per-session corr, n={len(_cmats)})')
_cb = _fig.colorbar(_im, ax=_ax, shrink=0.85); _cb.set_label('Pearson r')
_fig.tight_layout()
_fig.savefig(os.path.join(RESULTS_DIR, 'fig_elig_correlation_4x4.png'), dpi=200, bbox_inches='tight')
_fig.savefig(os.path.join(RESULTS_DIR, 'fig_elig_correlation_4x4.svg'), bbox_inches='tight')
plt.show()
print("Saved fig_elig_correlation_4x4.")
