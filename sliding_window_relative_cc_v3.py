...
#   'dot_prod_dev'  — CC - product_of_means baseline (v1 version)
#   'dot_prod_dev2' — sum_t r_pre(t) * (r_post(t) - mean_post_baseline)
#                     subtracts baseline from post BEFORE multiplication
CC_MODES = ['dot_prod_dev2']

# Rolling baseline (empty = none)
ROLLING_BASELINES = []
...
for mi in range(len(mice)):
    ...
    for sii in range(len(session_inds)):
        try:
            ...
            # Store raw dot_prod CC per pair per window per epoch
            raw_cc = np.full((n_wins, n_pairs, n_epochs), np.nan)
            # dev2: pre(t) * (post(t) - mean_post_baseline)
            ...
            # Baselines for deviation modes
            baseline_trials = np.arange(min(N_BASELINE, trl))
            baseline_pre = {}
            baseline_post = {}
            # dev2: mean postsynaptic activity per neuron over baseline (for subtraction before multiplication)
            baseline_post_mean = {}  # (n_neurons,) — mean activity per neuron over baseline trials
            for ei, ep in enumerate(EPOCH_ORDER):
                ...
                # Mean activity per neuron across baseline trials
                baseline_post_mean[ep] = np.nanmean(act_bl, axis=1)  # (n_neurons,)
            ...
            for wi, ws in enumerate(win_starts):
                ...
                for ei, ep in enumerate(EPOCH_ORDER):
                    ...
                    # dev2: pre(t) * (post(t) - mean_post_baseline)
                    post_dev = act[all_nt, :] - baseline_post_mean[ep][all_nt, np.newaxis]
                    cc_dev2 = np.sum(pre_act * post_dev, axis=1)  # (n_pairs,)
            ...
            for mode in CC_MODES:
                ...
                for ei, ep in enumerate(EPOCH_ORDER):
                    ...
                    elif mode == 'dot_prod_dev2':
                        # pre(t) * (post(t) - mean_post_baseline), precomputed
                        cc_all = dev2_cc[:, :, ei]
...
# CELL 4: Save
# ============================================================================
np.save(os.path.join(RESULTS_DIR, 'sliding_window_relative_cc_v2.npy'),
...
# ============================================================================
all_results = np.load(
    os.path.join(RESULTS_DIR, 'sliding_window_relative_cc_v2.npy'),
...
mode_colors = {'dot_prod': '#1f77b4', 'dot_prod_dev': '#ff7f0e', 'dot_prod_dev2': '#2ca02c'}
...
r_dev2 = np.full(n_s, np.nan)
for si in range(n_s):
    for mode, arr in [('dot_prod', r_raw), ('dot_prod_dev', r_dev), ('dot_prod_dev2', r_dev2)]:
        vals = all_results[mode][si]['hi_corr']
        v = vals[np.isfinite(vals)]
        if len(v) > 0:
            arr[si] = np.median(np.abs(v))

ax.scatter(r_raw, r_dev2, s=50, alpha=0.6, edgecolors='black', linewidth=0.5,
           color='#2ca02c', label='dev2 (post-baseline)')
ax.scatter(r_raw, r_dev, s=50, alpha=0.6, edgecolors='black', linewidth=0.5,
           color='#ff7f0e', label='dev (independence)', marker='s')
lim = max(np.nanmax(r_raw), np.nanmax(r_dev2), np.nanmax(r_dev)) * 1.1
...
with open(report_path, 'w', encoding='utf-8') as f:
    ...
    f.write("  dot_prod_dev2: pre(t) * (post(t) - mean_post_baseline), subtract post baseline before multiply\n\n")
