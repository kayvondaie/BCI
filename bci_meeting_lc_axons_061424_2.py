...
@author: kayvon.daie
"""
import os;os.chdir('E:/My Drive/Python Scripts/BCI_analysis/')
import re
...
for fi in range(3,4):
    ...
    for i in range(len(steps)):
        ...
        for si in range(len(steps[i])):
            ...
            v[ind] = 1
        vel[0:l,i] = v
    avg = np.nanmean(Faxons[:,0:40,:],axis=1)
    vel_row = []
    avg_row=[];
    ...
    plt.plot(avg_row[0:2000]);
    plt.plot(vel_row[0:2000])
    plt.show()
    

    plt.figure(figsize=(2, 4))  # Width: 10 inches, Height: 6 inches
    g=[]
    ...
    plt.subplot(211)
    plt.plot(np.nanmean(AVG,axis=1),'k')    
    boxoff()
    plt.subplot(212)
    plt.plot(np.nanmean(VEL,axis=1),'k')    
    boxoff()