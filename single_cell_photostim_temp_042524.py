...
folder = r'//allen/aind/scratch/BCI/2p-raw/BCI79/042524/'
ops = np.load(folder + r'suite2p_BCI/' +r'/plane0/ops.npy',allow_pickle = 'True').tolist()
...
out = ddc.read_stim_file(folder,r'suite2p_BCI/')
beam = -out['Beam']
dbeam = np.diff(medfilt(beam,55))
stimOn = np.where(dbeam>.02)[0]
one = np.array([stimOn[0]])
stimOn = np.concatenate((one,stimOn[np.diff(stimOn, prepend=np.nan) >= 600]))
...
s2pfolder = folder + '/suite2p_BCI/plane0/'
...
for ci in range(max(seq)):
    ...
    for ti in range(len(ind)):
        ...
        a[:,:,ti] = b
    A.append(a)
    amp = np.nanmean(a[21:24,:,:],axis = 0) - np.nanmean(a[15:19,:,:],axis = 0)  
    amp[np.isnan(amp)] = 0;
    h,p = stats.ttest_1samp(amp.T, 0)  
    k = np.where(stimDist[:,ci]>25)[0]
...
i = 0
plt.subplot(211)
plt.imshow(A[i][10:40,stim_cells[i],:].T,vmin=0,vmax=10)
plt.subplot(212)
...
B = []
for i in range(Fstim.shape[2]):
    ind = np.where((stimDist[:,i]>150) & (stimDist[:,i]<250))[0]
    B.append(np.nanmean(Fstim[:,ind,i],axis = 1))
B3 = np.asarray(B) 
plt.subplot(224)
plt.plot(t,np.nanmean(B,axis = 0)) 
plt.title('All cells 150-250 um from photostim')  
plt.xlabel('Time from photostim. (s)')
plt.tight_layout()
plt.show()
...
#lt.plot(t,np.nanmean(B3,axis = 0)) 
...
pre = 20
post = 40
all_stim_trials = np.zeros((pre+post,Ftrace.shape[0],len(frame_indices)))
target_amp = np.zeros(len(frame_indices))
for i in range(len(frame_indices)):
    cl = stim_cells[seq[i]-1]    
    indd = np.arange(frame_indices[i]-pre,frame_indices[i]+post)
    indd[indd<0] = 0
    indd[indd>Ftrace.shape[1]-1] = Ftrace.shape[1]-1
    target_amp[i] = np.nanmean(Ftrace[cl,indd[pre+1:]])-np.nanmean(Ftrace[cl,indd[0:pre-1]])
    all_stim_trials[:,:,i] = Ftrace[:,indd].T

plt.subplot(211)
plt.plot(target_amp)
plt.xlim(232,250)
plt.subplot(212)
plt.plot(np.diff(stimTime))
plt.xlim(232,250)
#%%
dt = 1/float(siHeader['metadata']['hRoiManager']['scanVolumeRate'])
t = np.arange(0,100+dt,dt)
t = t[0:all_stim_trials.shape[0]]
t = t - t[pre]
bad_trial = 244;
expected_cell = stim_cells[seq[243]-1]
plt.subplot(132)
plt.plot(t,all_stim_trials[:,expected_cell,bad_trial],'k',linewidth=.3)
plt.title('Trial #' + str(bad_trial)+' expected cell')

plt.show()
detected_stim = np.zeros(t_si.shape)
detected_stim[frame_indices[0:-2]] = 1
plt.subplot(211)
plt.plot(t_si,detected_stim)
plt.ylabel('Detected stim')
plt.xlim(244,245)
plt.subplot(212)
plt.plot(t_stim[2100000:2250000],dbeam[2100000:2250000])
plt.xlim(244,245)
plt.ylabel('Beam')
#%%
