...
for mouse in mice:
    ...
    for si in inds:
        ...
        # Per-trial end frame: trial start -> threshold crossing. For misses
        # (no crossing), use the full trial buffer.
        n_frames_per_trial = F.shape[0]
        end_frames = np.where(
            np.isfinite(tc_per_trial) & (tc_per_trial > 0),
            np.minimum((tc_per_trial / dt_si_s).astype(int),
                       n_frames_per_trial),
            n_frames_per_trial,
        ).astype(int)
        ...
        for ti in range(n_trials_s):
            ef = max(1, end_frames[ti])
            cn_per_trial[ti] = np.nanmean(F[:ef, cn, ti])
            non_cn_per_trial[ti] = np.nanmean(F[:ef, non_cn_mask, ti])
