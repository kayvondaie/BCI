...
NUM_STEPS,TST, CN, MOUSE, SESSION, FCNO = [], [], [], [],[], []
...
#%%
c = [np.corrcoef(x/np.nanstd(x),y)[0,1] for x,y in zip(HIb,RPE)]
hi = [np.nanmean(x/np.nanstd(x)) for x in HIb]
plt.scatter(c,hi,color = 'k');plt.xlabel('HI / RPE correlation');plt.ylabel('Session avg HI')
...
vel = [x/y for x,y in zip(NUM_STEPS[0:38],rt[0:38])]
vel_rpe = []
for i in range(len(vel)):
    a = compute_rpe_standard(vel[i],4,1)
    vel_rpe.append(-a)
        
        
        
hi = [(x/np.nanstd(x)) for x in HIb[0:38]]
x = np.concatenate(vel)
y = np.concatenate(hi)
ind = np.where(y!=0)[0]
pf.mean_bin_plot(y,x,15);plt.ylabel('Speed');plt.xlabel('HI')
plt.show()
#%%
c = []
hi_avg = [np.nanmean(x) for x in hi]
for i in range(len(hi)):
    x = hi[i]
    y = vel_rpe[i]
    ind = np.where(x!=0)[0]
    c.append(np.corrcoef(x,y)[0,1])
plt.scatter(c,hi_avg)