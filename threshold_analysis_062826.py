...
for mi in range(len(mice)):
    ...
    for sii in range(len(session_inds)):
        ...
        try:
            ...
            # Cursor speed under fixed-reference thresholds, active window.
            # Hardest = max headroom -> sensitive to CN increases (no ceiling).
            # Easiest (~epoch 1, the capsule reference) -> CN often exceeds the
            # low upper, so transfer_fun clips at max_speed -> ceiling-limited.
            hardest_lower_s = np.nanmax(thr_l)
            hardest_upper_s = np.nanmax(thr_u)
            easiest_lower_s = np.nanmin(thr_l)
            easiest_upper_s = np.nanmin(thr_u)
            cursor_speed_s = np.full(trl, np.nan)
            cursor_speed_easy_s = np.full(trl, np.nan)
            cursor_peak_s = np.full(trl, np.nan)   # 90th-pct drive (burst, hardest thr)
            for ti_sp in range(min(trl, len(cn_fluor))):
                fl_sp = cn_fluor[ti_sp][:cn_stp[ti_sp]]   # active window: trial start -> crossing
                if len(fl_sp) > 0:
                    spd_hard = transfer_fun(fl_sp, hardest_lower_s, hardest_upper_s,
                                            low_floor=low_floor_for(mouse))
                    cursor_speed_s[ti_sp] = np.nanmean(spd_hard)
                    cursor_peak_s[ti_sp] = np.nanpercentile(spd_hard, 90)  # peak drive
                    cursor_speed_easy_s[ti_sp] = np.nanmean(
                        transfer_fun(fl_sp, easiest_lower_s, easiest_upper_s,
                                     low_floor=low_floor_for(mouse)))
...
#%% ============================================================================
# CELL 062826: pooled DRIVE (cursor speed) + BEHAVIOR (actual - expected HR)
# ============================================================================
# Built entirely on the validated CELL 1-4 quantities:
#   Drive  = per-trial cursor_speed = mean transfer_fun(CN, hardest thresholds)
#            -> includes BOTH saturation (clip) and the noise floor; fixed
#            (hardest) reference, so it's non-circular. (all_session_trials)
#   Behav  = MA(hit) - expected_hr_correct, where expected_hr_correct is the
#            transfer-function expected hit rate (saturation+floor). (all_epoch_stats)
# Pooled across sessions by trial index (mean +/- SEM), cut where < MIN_SESS.
from scipy.stats import wilcoxon as _wilcoxon

AGG_MA, AGG_BASE_N, AGG_MIN_SESS, AGG_SHOW = 10, 20, 10, 120
...
drive_hard, drive_easy, drive_peak, behav_tr = [], [], [], []
for (mouse_a, session_a), st in all_session_trials.items():
    hit_a = np.asarray(st['hit'], float)
    n_a = len(hit_a)
    exp_a = np.full(n_a, np.nan)
    for r in all_epoch_stats:
        if (r['mouse'] == mouse_a and r['session'] == session_a
                and np.isfinite(r.get('expected_hr_correct', np.nan))):
            exp_a[r['trial_start']:r['trial_end']] = r['expected_hr_correct']
    behav_tr.append(_aggmov(hit_a, AGG_MA) - exp_a)
    for store, key in [(drive_hard, 'cursor_speed'), (drive_easy, 'cursor_speed_easy'),
                       (drive_peak, 'cursor_peak')]:
        dr = _aggmov(np.asarray(st[key], float), AGG_MA)
        b = np.nanmean(dr[:AGG_BASE_N])
        store.append(dr - b if np.isfinite(b) else dr)

def _aggpool(traces):
    M = np.full((len(traces), AGG_SHOW), np.nan)
    for i, t in enumerate(traces):
        k = min(len(t), AGG_SHOW); M[i, :k] = t[:k]
    nT = np.sum(~np.isnan(M), axis=0)
    return (np.nanmean(M, axis=0),
            np.nanstd(M, axis=0) / np.sqrt(np.clip(nT, 1, None)), nT)

def _late3(t):
    t = np.asarray(t, float); return np.nanmean(t[len(t) * 2 // 3:])

x = np.arange(AGG_SHOW)
fig_agg, axes_agg = plt.subplots(1, 2, figsize=(7.0, 2.4))

# --- behavior panel ---
axB = axes_agg[0]
m, sem, nT = _aggpool(behav_tr); keep = nT >= AGG_MIN_SESS
axB.fill_between(x[keep], (m - sem)[keep], (m + sem)[keep], color='crimson', alpha=0.2, lw=0)
axB.plot(x[keep], m[keep], color='crimson', lw=1.2)
axB.axhline(0, color='k', lw=0.6, ls='--', alpha=0.4)
ltB = np.array([_late3(t) for t in behav_tr]); ltB = ltB[np.isfinite(ltB)]
pB = _wilcoxon(ltB).pvalue if len(ltB) > 1 else np.nan
axB.text(0.03, 0.97, f'p={pB:.3f}', transform=axB.transAxes, ha='left', va='top')
axB.set_xlabel('Trial #'); axB.set_ylabel('actual - expected HR')
axB.set_title('Behavior (transfer-fn expected)', fontsize=9)
axB.spines[['top', 'right']].set_visible(False)

# --- drive panel: hardest vs easiest fixed reference ---
axD = axes_agg[1]
for traces, col, lab in [(drive_hard, 'crimson', 'mean (hardest)'),
                         (drive_peak, 'darkorange', 'peak90 (hardest)'),
                         (drive_easy, 'steelblue', 'mean (easiest)')]:
    m, sem, nT = _aggpool(traces); keep = nT >= AGG_MIN_SESS
    axD.fill_between(x[keep], (m - sem)[keep], (m + sem)[keep], color=col, alpha=0.18, lw=0)
    axD.plot(x[keep], m[keep], color=col, lw=1.2, label=lab)
    lt = np.array([_late3(t) for t in traces]); lt = lt[np.isfinite(lt)]
    p = _wilcoxon(lt).pvalue if len(lt) > 1 else np.nan
    print(f"  drive ({lab}) late-third vs 0: p={p:.4f}")
axD.axhline(0, color='k', lw=0.6, ls='--', alpha=0.4)
axD.set_xlabel('Trial #'); axD.set_ylabel('cursor speed - baseline')
axD.set_title('Drive (cursor speed, sat+floor)', fontsize=9)
axD.legend(frameon=False, fontsize=7, loc='upper left')
axD.spines[['top', 'right']].set_visible(False)
plt.tight_layout()
fname_agg = 'drive_behavior_062826'
fig_agg.savefig(os.path.join(PANEL_DIR, f'{fname_agg}.png'), dpi=300)
fig_agg.savefig(os.path.join(PANEL_DIR, f'{fname_agg}.svg'))
print(f"pooled {len(behav_tr)} sessions -> {fname_agg}")

#%% ============================================================================
# CELL 062826b: SWITCH-ALIGNED drive + behavior (local pre-switch baseline)
# ============================================================================
# For each threshold INCREASE, align to the switch (trial 0 = first trial of the
# new, harder epoch) and subtract the mean of the PRE_SW pre-switch trials. The
# local baseline cancels the session-global CN decline / photobleaching that
# swamped the absolute-trial curve. Compensation = drive climbs back up after
# the switch; behavior = hit rate drops at the switch then recovers.
# Caveat: thresholds are often raised at a performance peak, so the immediate
# pre-switch point can be a local high (regression-to-mean) -> read the POST-
# switch RECOVERY slope, not just the step.
PRE_SW, POST_SW, MIN_SW = 8, 15, 8
xrel = np.arange(-PRE_SW, POST_SW)

def _collect_switch(metric_key, use_ma=False):
    segs = []
    for (m_, s_), st in all_session_trials.items():
        sw = np.asarray(st['switches'], int)
        thr_u = np.asarray(st['thr_u'], float)
        xk = np.asarray(st[metric_key], float)
        if use_ma:
            xk = _aggmov(xk, AGG_MA)
        n = len(xk)
        for k in range(1, len(sw)):
            s = int(sw[k])
            if s < 1 or s >= n or not (thr_u[s] > thr_u[s - 1]):   # increases only
                continue
            pre = xk[max(0, s - PRE_SW):s]; pre = pre[np.isfinite(pre)]
            if len(pre) < 3:
                continue
            base = np.mean(pre)
            seg = np.full(PRE_SW + POST_SW, np.nan)
            for r in range(-PRE_SW, POST_SW):
                t = s + r
                if 0 <= t < n and np.isfinite(xk[t]):
                    seg[r + PRE_SW] = xk[t] - base
            segs.append(seg)
    return np.array(segs)

def _sw_pool(segs):
    nT = np.sum(np.isfinite(segs), axis=0)
    return (np.nanmean(segs, axis=0),
            np.nanstd(segs, axis=0) / np.sqrt(np.clip(nT, 1, None)), nT)

fig_sw, ax_sw = plt.subplots(1, 2, figsize=(7.0, 2.4))

# left: behavior (actual hit rate, MA), switch-aligned
segsH = _collect_switch('hit', use_ma=True)
mH, semH, nH = _sw_pool(segsH); keepH = nH >= MIN_SW
ax_sw[0].fill_between(xrel[keepH], (mH - semH)[keepH], (mH + semH)[keepH],
                      color='crimson', alpha=0.2, lw=0)
ax_sw[0].plot(xrel[keepH], mH[keepH], color='crimson', lw=1.2)
ax_sw[0].axvline(0, color='k', lw=0.8, ls=':'); ax_sw[0].axhline(0, color='k', lw=0.6, ls='--', alpha=0.4)
ax_sw[0].set_xlabel('Trial from threshold increase'); ax_sw[0].set_ylabel('hit rate - pre-switch')
ax_sw[0].set_title('Behavior', fontsize=9); ax_sw[0].spines[['top', 'right']].set_visible(False)

# right: drive traces switch-aligned
axS = ax_sw[1]
for key, col, lab in [('cursor_speed', 'crimson', 'mean (hardest)'),
                      ('cursor_peak', 'darkorange', 'peak90 (hardest)'),
                      ('cursor_speed_easy', 'steelblue', 'mean (easiest)')]:
    segs = _collect_switch(key)
    m, sem, nT = _sw_pool(segs); keep = nT >= MIN_SW
    axS.fill_between(xrel[keep], (m - sem)[keep], (m + sem)[keep], color=col, alpha=0.16, lw=0)
    axS.plot(xrel[keep], m[keep], color=col, lw=1.2, label=lab)
    post = np.nanmean(segs[:, PRE_SW + 1:PRE_SW + 1 + POST_SW], axis=1)
    post = post[np.isfinite(post)]
    pp = _wilcoxon(post).pvalue if len(post) > 1 else np.nan
    print(f"  switch-aligned drive ({lab}) post vs pre-baseline: p={pp:.4f}, n_sw={len(post)}")
axS.axvline(0, color='k', lw=0.8, ls=':'); axS.axhline(0, color='k', lw=0.6, ls='--', alpha=0.4)
axS.set_xlabel('Trial from threshold increase'); axS.set_ylabel('cursor speed - pre-switch')
axS.set_title('Drive', fontsize=9); axS.legend(frameon=False, fontsize=7, loc='lower right')
axS.spines[['top', 'right']].set_visible(False)
plt.tight_layout()
fname_sw = 'switch_aligned_drive_062826'
fig_sw.savefig(os.path.join(PANEL_DIR, f'{fname_sw}.png'), dpi=300)
fig_sw.savefig(os.path.join(PANEL_DIR, f'{fname_sw}.svg'))
print(f"switch-aligned: {len(segsH)} threshold increases -> {fname_sw}")
