...
#%%
def build_k_concat_for_trials(F, tsta, data, trials, epoch):
    """
    Return k_concat shaped (samples, neurons) for a given list/array of trial indices.
    Uses the same epoch logic as your main code.
    """
    trials = np.asarray(trials, dtype=int)
    if trials.size == 0:
        return np.zeros((0, F.shape[1]), dtype=float)

    # figure out tpts for these trials
    if epoch in ('step', 'reward'):
        if epoch == 'step':
            step_list = [s for s in data['step_time'][trials] if len(s) > 0]
        else:
            step_list = [s for s in data['reward_time'][trials] if len(s) > 0]
    ...
    # slice and reshape: (tpts, neurons, trials) -> (samples, neurons)
    kk = F[:, :, trials][tpts, :, :]
    kk = kk.transpose(2, 0, 1).reshape(-1, kk.shape[1])

    # keep rows where at least something is finite
    m = np.isfinite(kk).all(axis=1)
    kk = kk[m, :]

    # replace remaining nans (should be none after all(axis=1), but safe)
    kk = np.nan_to_num(kk, nan=0.0)
    return kk
...
for mi in range(len(mice)):
    ...
    epoch         =  'trial_start'  # reward, step, trial_start,pre
    TR            =   2
    ...
    #for sii in range(si,si+2):
        ...
        for i in range(len(trial_bins) - 1):
            ...
            if pairwise_mode == 'noise_corr':
                ...
                TR = 50  # number of previous trials used to define causal baseline
            
                # Current-bin activity (samples × neurons)
                k_src = k_concat
            
                # --- causal baseline for postsynaptic term ---
                t0 = int(trial_bins[i])                 # first trial index in current bin
                prev_start  = max(0, t0 - TR)
                prev_trials = np.arange(prev_start, t0) # strictly earlier trials
            
                k_hist = build_k_concat_for_trials(F, tsta, data, prev_trials, epoch)
            
                if k_hist.shape[0] >= 5:
                    mu_post = np.mean(k_hist, axis=0)   # (neurons,)
                else:
                    mu_post = (
                        np.mean(k_concat, axis=0)
                        if k_concat.shape[0] > 0
                        else np.zeros(F.shape[1])
                    )
            
                # Postsynaptic deviation only
                k_post_dev = k_concat - mu_post[None, :]
            
                denom = max(1, k_concat.shape[0])
                CCts[:, :, i] = (k_src.T @ k_post_dev) / denom
        ...
        for i in range(CC.shape[2]):
            ...
            for gi in range(stimDist.shape[1]):
                ...
                if len(cl)>0:
                    ...
                    nontarg = np.where((stimDist[:,gi]>30)&(stimDist[:,gi]<100))
                    y = AMP[1][nontarg,gi]
                    yo = AMP[0][nontarg,gi]
                    Y.append(y)
                    Yo.append(yo)
...
#%%
rt = [x[:] for x in RT]
hi = [hib[:]/np.nanstd(hib[0:]) for hib in HIb]

_,_,p = pf.mean_bin_plot_shade(np.concatenate(rt),np.concatenate(hi),5,1,1,'k','k',.2);
