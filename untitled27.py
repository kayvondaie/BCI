...
for sii in range(si,si+1):
    ...
    init_taus = np.array([10, 10, 10])
    
    res = minimize(
        objective_loss,
        x0=init_taus,
        bounds=[(1, 20)] * 3,
        args=(rt, hit, trial_bins, Xstep_T, Xrew_T, Xts_T, Y_T)
    )
    
    best_taus = res.x
    print(f"Best taus: {best_taus}")
    
    # Now recompute behavior_features using optimized taus
    behavior_features = compute_behavior_features_from_taus(rt, hit, trial_bins, best_taus)
    
    # Normalize behavior features (again)
...
#%%
def compute_behavior_features_from_taus(rt, hit, trial_bins, taus):
    taus = np.nan_to_num(taus, nan=10.0)
    tau_rt_rpe, tau_hit_rpe, tau_miss_rpe = taus

    def compute_rpe(signal, baseline, tau, fill_value):
        rpe = np.full_like(signal, np.nan, dtype=np.float64)
        signal_clean = np.where(np.isnan(signal), fill_value, signal)

        for i in range(len(signal)):
            if i == 0:
                avg = baseline
            else:
                start = max(0, i - int(tau))
                avg = np.nanmean(signal_clean[start:i]) if i > start else baseline
            rpe[i] = signal_clean[i] - avg
        return rpe

    rt_rpe = -compute_rpe(rt, baseline=2.0, tau=tau_rt_rpe, fill_value=10)
    hit_rpe = compute_rpe(hit, baseline=0, tau=tau_hit_rpe, fill_value=0)
    miss_rpe = compute_rpe(hit == 0, baseline=0, tau=tau_miss_rpe, fill_value=1)

    rt_bin, rpe_bin, hit_bin, hit_rpe_bin, miss_bin, miss_rpe_bin = [], [], [], [], [], []
    for i in range(len(trial_bins)):   # <- NOT len(trial_bins)-1
        ind = np.arange(trial_bins[i-1] if i > 0 else 0, trial_bins[i])
        rt_bin.append(np.nanmean(rt[ind]))
        rpe_bin.append(np.nanmean(rt_rpe[ind]))
        hit_bin.append(np.nanmean(hit[ind]))
        hit_rpe_bin.append(np.nanmean(hit_rpe[ind]))
        miss_bin.append(np.nanmean((hit[ind]==0).astype(float)))
        miss_rpe_bin.append(np.nanmean(miss_rpe[ind]))
...
def objective_loss(taus, rt, hit, trial_bins, Xstep_T, Xrew_T, Xts_T, Y_T, alpha=100):
    behavior_features = compute_behavior_features_from_taus(rt, hit, trial_bins, taus)
    ...
    for train_idx, test_idx in kf.split(X_T):
        ...
        r_test, _ = pearsonr(Y_test_pred, Y_test)
        corr_test.append(r_test)

    return -np.mean(corr_test)  # Negative for minimization
