"""
Three-factor variance explained, inhibitory-photostim version.

For each session that passes the inhibitory-photostim CSV QC filter, predicts
the per-(target, non-target) change in photostim weight (dW = amp_post − amp_pre)
from the pre-cue coactivity (target rate × non-target rate-deviation), summed
in sliding windows across trials and averaged across non-target neurons. Fits
with k-fold cross-validation.

Adapted from pan_neuronal_analysis2/three_factor_variance_explained_by_group.py
with four updates for the inhibitory data:
  • compute_amp_from_photostim_artifact_free (no light-leakage threshold)
    plus per-session h5 stim_params loading.
  • CSV-based QC (Type=Inhibitory, Pre OR Post = Good/Ok, no non-zero offset notes).
  • Pickle-tolerant decoder for step_time / reward_time.
  • The outlier vs rest non-target split is dropped — directly-stimmed cells
    are already known to be I cells in this paradigm, so we just analyze all
    non-target neurons together.
"""
