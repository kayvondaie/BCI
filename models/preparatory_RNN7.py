...
vlim = np.abs(dw_fit).max()
sns.heatmap(dw_fit.T, xticklabels=labels, yticklabels=labels, annot=False, fmt=".2f",
...
vlim = np.abs(dw_corr).max()
sns.heatmap(dw_corr.T, xticklabels=labels, yticklabels=labels, annot=False, fmt=".2f",
            cmap='bwr', vmin=-vlim, vmax=vlim)
