# -*- coding: utf-8 -*-
"""
Created on Thu Jul 24 12:46:57 2025
...
for epoch in range(100):
    ...
    for ti in range(T):
        r_list.append(r_t)
        inp = torch.relu(r_t) @ W + ext_noise[ti]
        dr = (-r_t + inp) * dt / tau
        r_t = r_t + dr
    ...
    loss = nn.functional.mse_loss(torch.relu(r[300:, cond_idx]), target[300:])

    if penalize:
        delta_r = torch.relu(r[1:]) - torch.relu(r[:-1])
...
for j in range(2):
    ...
    plt.plot(t, np.mean(fun(r[:, pre_idx]), axis=1), label='Pre neurons')
    plt.plot(t, np.mean(fun(r[:, trl_idx]), axis=1), label='Trial neurons')
    plt.plot(t, np.mean(fun(r[:, others_idx]), axis=1), label='Others')
    plt.plot(t, fun(r[:, cond_idx]), label='CN')
...
for ei in range(2):
    ...
    for ci in range(N):
        ...
        r = np.zeros((51,N))
        for ti in range(50):
            inp = dt/tau*(-r[ti,:] + fun(r[ti,:])@ww + stim*ps_inp[ti] + x_noise[ti] * inpW)
            r[ti+1,:] = r[ti,:] + inp
        R[:,ci,:] = r
...
plt.subplot(3,2,6)
cce = np.stack([np.corrcoef(r[300:,:].T) for r in r_epoch[0:10]])
ccl = np.stack([np.corrcoef(r[300:,].T) for r in r_epoch[-10:]])
dcc = np.nanmean(ccl,0) - np.nanmean(cce,0);
x = dcc.flatten()
y = dw.flatten()
