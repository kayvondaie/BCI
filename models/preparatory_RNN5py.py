...
# Initial W
W_np = np.zeros((N, N))
W_np[np.ix_(pre_idx, pre_idx)] = np.random.randn(40, 40) * 0.5 + .4
W_np[np.ix_(trl_idx, trl_idx)] = np.random.randn(40, 40) * 0.5 + .7
W_np[np.ix_(pre_idx, trl_idx)] = np.random.randn(40, 40) * 1.0 + 1
