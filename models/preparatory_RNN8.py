...
W_np = W_np / np.max(np.real(np.linalg.eigvals(W_np)))*.4
...
for epoch in range(100):
    ...
    ext_noise = external + torch.randn_like(external) * .05
...
plt.title('Lasso Δ (W_final − W_np)')

plt.subplot(122)
dw_corr = w_corr[1] - w_corr[0]
dw_corr_reordered = dw_corr[np.ix_(order, order)]
vlim = np.abs(dw_corr_reordered).max()
sns.heatmap(dw_corr_reordered.T, xticklabels=reordered_labels, yticklabels=reordered_labels,
            annot=False, fmt=".2f", cmap='bwr', vmin=-vlim, vmax=vlim)
plt.ylabel("Postsynaptic Feature")
plt.xlabel("Presynaptic Feature")
plt.title('Correlation Δ (W_final − W_np)')
