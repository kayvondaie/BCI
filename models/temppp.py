...
RE,RL,WCC_avg,DWCC,BEF,AFT,DWCC_flat,DCC = [],[],[],[],[],[],[],[]
# Constants
train_inputs =  False
N = 100
dt = 0.01
...
for rep in range(1):
    ...
    inpweights = torch.nn.Parameter(inpweights, requires_grad=train_inputs)
    ...
    W_np = W_np / np.max(np.real(np.linalg.eigvals(W_np))) * 0.95
    np.fill_diagonal(W_np, 0)
    
    W = torch.tensor(W_np, dtype=torch.float32, device=device)
    W = torch.nn.Parameter(W.clone(), requires_grad=True)  # force fresh param
    ...
    for epoch in range(150):
        ...
        for ti in range(T):
            ...
            dr = (-r_t + torch.clamp(inp, min=0, max=3)) * dt / tau
            #dr = (-r_t + torch.nn.functional.leaky_relu(inp, negative_slope=0.01)) * dt / tau
            r_t = r_t + dr
    ...
    for ei in range(2):
        ...
        for ci in range(N):
            ...
            r = np.zeros((51,N))
            for ti in range(50):
                inp = r[ti,:] @ ww + x_noise[ti] * inpW
                dr = (-r[ti,:] + fun(inp, 0, 1) + stim * ps_inp[ti]) * dt / tau
                r[ti+1,:] = r[ti,:] + dr
    
            R[:,ci,:] = r
        Direct.append(np.nanmean(R[:, np.arange(N), np.arange(N)],0))
        wcc = np.nanmean(R,0) 
        ...
        if cv == True:            
            lasso_cv = LassoCV(alphas=np.logspace(-8, -2, 10), 
                               cv=5,                           
                               fit_intercept=False,
                               max_iter=10000)
            lasso_cv.fit(X_clean, wcc_flat)
    ...
    WCC_avg.append(dw_fit_reordered)
    
    
    plt.subplot(3,2,6)
    cce = np.corrcoef(np.stack([np.nanmean(r[300:,:],0) for r in r_epoch[0:30]]).T)
    ccl = np.corrcoef(np.stack([np.nanmean(r[300:,:],0) for r in r_epoch[-20:]]).T)
    dcc = ccl - cce
    x = dcc.flatten()
    y = (WCC[1] - WCC[0]).flatten()
    #y = (W_final - W_np).flatten()
    ind = np.where((np.isnan(x)==0) & (np.eye(N).flatten()==0))[0]
    
    
    pf.mean_bin_plot(x[ind],y[ind],5)
    DWCC_flat.append(y[ind])
    DCC.append(x[ind])
...
#%%
plt.subplot(323)    
wcc_flat = WCC[0].flatten()
wcc_flat = W_final.flatten()
vectors = [all_ones, trl, pre, dpre*0, dtrl*0]
X = []
for i in range(len(vectors)):
    for j in range(len(vectors)):
        a = np.outer(vectors[i], vectors[j])
...
vlim = np.abs(dw_fit_reordered).max()
vlim = 3e-3
b = dw_fit_reordered.T
a = b[[0,1,3],:][:,[0,1,3]]
WCC_avg.append(a)
reordered_labels = ["All", "Pre", "Trl"]
sns.heatmap(a, xticklabels=reordered_labels, yticklabels=reordered_labels,
            annot=False, fmt=".2f", cmap='bwr', vmin=-vlim, vmax=vlim)
DWCC.append(dw_fit_reordered)
plt.xlabel("Presynaptic Feature")
plt.title('$W_{cc}$')
...
#%% Combined analysis across reps
import matplotlib.pyplot as plt
import seaborn as sns
import numpy as np

# --- Compute means across reps ---
RE_mean = np.mean(np.stack(RE, axis=0), axis=0)
RL_mean = np.mean(np.stack(RL, axis=0), axis=0)
BEF_mean = np.mean(np.stack(BEF, axis=0), axis=0)
AFT_mean = np.mean(np.stack(AFT, axis=0), axis=0)
WCC_mean = np.mean(np.stack(WCC_avg, axis=0), axis=0)
DWCC_mean = np.mean(np.stack(DWCC, axis=0), axis=0)

# Flatten and concatenate DCC and DWCC_flat across reps
x_all = np.concatenate(DCC)
y_all = np.concatenate(DWCC_flat)

# Plot average activity traces
plt.figure(figsize=(6, 8))
plt.subplot(3,2,1)
plt.plot(t, np.mean(RE_mean[:, pre_idx], axis=1), label='Pre neurons')
plt.plot(t, np.mean(RE_mean[:, trl_idx], axis=1), label='Trial neurons')
plt.plot(t, np.mean(RE_mean[:, others_idx], axis=1), label='Others')
plt.plot(t, RE_mean[:, cond_idx], label='CN')
plt.xlabel('Time')
plt.ylabel('Mean activity')
plt.title('Early trial')
plt.legend()

plt.subplot(3,2,2)
plt.plot(t, np.mean(RL_mean[:, pre_idx], axis=1), label='Pre neurons')
plt.plot(t, np.mean(RL_mean[:, trl_idx], axis=1), label='Trial neurons')
plt.plot(t, np.mean(RL_mean[:, others_idx], axis=1), label='Others')
plt.plot(t, RL_mean[:, cond_idx], label='CN')
plt.xlabel('Time')
plt.ylabel('Mean activity')
plt.title('Late trial')

# Scatter: early vs late activity
plt.subplot(3,2,5)
plt.plot(BEF_mean, AFT_mean, 'ko')
plt.plot(BEF_mean[cond_idx], AFT_mean[cond_idx], 'mo')
...
plt.subplot(3,2,3)
vlim = 3e-3
sns.heatmap(WCC_mean, xticklabels=reordered_labels, yticklabels=reordered_labels,
...
plt.subplot(3,2,4)
vlim = 3e-4
sns.heatmap(DWCC_mean.T, xticklabels=reordered_labels, yticklabels=reordered_labels,
            cmap='bwr', vmin=-vlim, vmax=vlim)
plt.xlabel("Presynaptic Feature")
plt.title('$\Delta W_{cc}$')

# ΔCorrelation vs ΔWcc
plt.subplot(3,2,6)
pf.mean_bin_plot(x_all, y_all, 5)
plt.xlabel('$\Delta$ Correlation')
plt.ylabel('$\Delta W_{cc}$')
...
plt.subplot(1,3,1)
plt.plot(t, RE[0][:, cond_idx], color=[1, .7, .7])  # red colorplt.plot(t, RE[0][:,cond_idx],color = [1 0 0])
plt.plot(t, RL[0][:, cond_idx], color=[1, 0, 0])  # red color
plt.xlim((-2,3))
plt.xlabel('Time')
plt.ylabel('Mean activity')
...
trl_amp = np.stack([np.nanmean(r[300:,:],0) for r in r_epoch[29:131]])
x = np.arange(trl_amp.shape[0])
slopes = np.array([np.polyfit(x, trl_amp[:, i], 1)[0] for i in range(trl_amp.shape[1])])
b = np.argsort(slopes);
bb = np.where(b == cond_idx)[0]
plt.subplot(132)
plt.plot(slopes[b],'go')
plt.plot([bb, bb], [0, slopes[b[bb]][0]], 'ro-')
plt.xlabel('Sorted neuron idx')
plt.ylabel('Trial slope (AU)')
plt.tight_layout()
plt.show()
...
# Assume X_clean and wcc_flat are already defined and preprocessed
alphas = np.logspace(-8, -2, 100)
lasso_cv = LassoCV(alphas=alphas, cv=5, fit_intercept=False, max_iter=10000)
lasso_cv.fit(X_clean, wcc_flat)

# Plot the mean CV error for each alpha
mean_mse = np.mean(lasso_cv.mse_path_, axis=1)
std_mse = np.std(lasso_cv.mse_path_, axis=1)

plt.figure(figsize=(6, 4))
plt.semilogx(lasso_cv.alphas_, mean_mse, label='Mean CV MSE')
plt.xlabel('Alpha')
plt.ylabel('MSE')
plt.tight_layout()
plt.show()
