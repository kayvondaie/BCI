# -*- coding: utf-8 -*-
"""
Created on Sun May 18 16:51:10 2025
...
RPE_FIT = []
AVG_RPE = []
CC_RPE, CC_RT, CC_HIT, HIT_RATE, CN_SNR, D_HIT_RATE, RPE_VAR, CORR_RPE, CORR_RT, RT_WINDOW, HIT_WINDOW, THR_WINDOW, CORR_HIT = [],[],[],[],[],[],[],[],[],[],[],[],[]
...
for mi in range(len(mice)):
    ...
    #for sii in range(si,si+1):
        ...
        # Boolean arrays
        hit_mask = ~np.isnan(rt)   # True where there was a reward
        miss_mask = np.isnan(rt)   # True where there was no reward

        # First difference to detect new hits or misses
        hit = np.concatenate(([0], np.diff(hit_mask.astype(int))))
        miss = np.concatenate(([0], np.diff(miss_mask.astype(int))))
        rt[np.isnan(rt)] = 20;
        ...
        miss = compute_rpe(miss_mask, baseline=miss_mask[0], window=2, fill_value=50)
        hit = compute_rpe(hit_mask, baseline=hit_mask[0], window=2, fill_value=50)
        ...
        for i in range(len(trial_bins)-1):
            ...
            miss_bin[i] = np.nanmean(miss[ind])
        ...
        cc_hit = (np.corrcoef(hit_bin[:-1],np.asarray(b)[:-1])[1,0])
        cc_miss = (np.corrcoef(miss_bin[:-1],np.asarray(b)[:-1])[1,0])
        plt.bar(['RPE', 'RT', 'HIT',"MISS"], [cc_rpe, cc_rt, cc_hit, cc_miss],color = 'k')
        plt.ylabel('Correlation with b')
        plt.title('Correlation of b with RPE and RT')
        
        CC_RPE.append(cc_rpe)
        CC_RT.append(cc_rt)
        CC_HIT.append(cc_hit)
        ...
        corr_hit = np.zeros(len(b)-window)
        ...
        for i in range(len(b)-window):
            ...
            corr_hit[i] = np.corrcoef(b[i:i+window],hit_bin[i:i+window])[0,1]            
...
t_stat, p = ttest_rel(np.nanmean(A_array[0:1,:],axis=0), np.nanmean(A_array[-1:,:],axis=0));print(p)
#%%
dhit = []
drt  = []
for i in range(len(THR)):
    ...
    if len(ind) > 0:
        ind = ind[0]
        d = np.nanmean(HIT[i][ind-1:ind]) - np.nanmean(HIT[i][ind+1:ind+7])
        dd = np.nanmean(RT[i][ind-1:ind]) - np.nanmean(RT[i][ind+1:ind+7])
        dhit.append(d)
        drt.append(dd)
    else:
        dhit.append(np.nan)
