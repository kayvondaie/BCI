...
folder = r'G:/My Drive/Learning rules/BCI_data/ai230/BCI79/042324/'
...
data = dict()
data['photostim'] = dict()
data['photostim']['Fstim'], data['photostim']['seq'], data['photostim']['favg'], data['photostim']['stimDist'], data['photostim']['stimPosition'], data['photostim']['centroidX'], data['photostim']['centroidY'], data['photostim']['slmDist'],data['photostim']['stimID'],_,_ = ddc.stimDist_single_cell(ops, Ftrace,siHeader,stat)
#%%
...
for i in range(15):
    plt.subplot(3,5,i+1)
    plt.imshow(A[i][10:40,stim_cells[i],:].T,vmin=0,vmax=4)
plt.show()
for i in range(15):
    plt.subplot(3,5,i+1)
    plt.plot(t,Fstim[:,stim_cells[i],i],'k')
plt.show()
for i in range(15):
    plt.subplot(3,5,i+1)
    plt.plot(t,A[i][:,stim_cells[i],:],linewidth=.2)
plt.show()
#%%
...
B = []
for i in range(Fstim.shape[2]):
    ind = np.where((stimDist[:,i]>300) & (stimDist[:,i]<40000) & (cell_ind<300))[0]
    B.append(np.nanmean(Fstim[:,ind,i],axis = 1))
B3 = np.asarray(B) 
plt.plot(t,np.nanmean(B,axis = 0)) 
...
i = 4
plt.subplot(4,3,1)
plt.plot(t,A[i][:,stim_cells[i],:],linewidth=.5)
ind = np.where((stimDist[:,i]>30) & (amp[:,i]>1))[0]
for j in range(8):
    plt.subplot(4,3,j+2)
    ci = ind[j]
    plt.plot(t,A[i][:,ci,:],linewidth=.2)
