...
hit_gains = [0.5, 2]
ts_gains  = [0.5, 2]
step_taus = [int(round(x / dt_si)) for x in [60, 130]]
hit_taus  = [int(round(x / dt_si)) for x in [60, 130]]
...
for sii in range(si,si+1):
    ...
    best_score = -np.inf
    for step_gain, hit_gain, ts_gain, step_tau, hit_tau, ts_tau in tqdm(param_grid):
        # RPE
        ...
        ts_rate = compute_ema(ts_vector, round(1 / dt_si), 0)
        ts_rate_slow = compute_ema(ts_vector, ts_tau, 0)
        ...
        rpe_ts = (ts_rate - np.nanmean(ts_rate)) / np.nanstd(ts_rate) \
                - ts_gain * (ts_rate_slow - np.nanmean(ts_rate_slow)) / np.nanstd(ts_rate_slow)
        ...
        X_T = np.vstack([Xstep, Xrew, Xts]).T
    ...
    #axs[0].plot(rpe_step * beta_avg[0], color='k', linewidth=1)
    axs[0].plot(rpe_step * beta_avg[0] + rpe_hit * beta_avg[1] + rpe_ts * beta_avg[2], color='k', linewidth=1)
    axs[0].set_title("Total RPE (z-scored)")
    axs[0].set_ylabel("RPE")
    axs[0].set_xlabel("Frame")

    # 2. Beta weights
    axs[1].bar(['Step RPE', 'Hit RPE', 'Trial Start RPE'], beta_avg, color='gray', edgecolor='k')
    ...
    plt.suptitle("Mouse:" + data['mouse'] + "," + data['session'])
    plt.show()
    
    name = mouse + '_RPE_fitting'
