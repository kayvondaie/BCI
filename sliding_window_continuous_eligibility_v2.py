...
# If True (default), post-neuron activity is baseline-subtracted in the
# eligibility: E = pre * (r_post - h_bar).  If False, no baseline is removed
# and E = pre * r_post directly.  Setting this to False turns off the effect
# of TAU_BL entirely.
SUBTRACT_POST_BASELINE = True
...
# When SUBTRACT_POST_BASELINE is off, append "_nobl" so those runs don't
# overwrite the with-baseline runs.
CONFIG_TAG = (f"_te{TAU_ELIG:g}_tb{TAU_BL:g}_tr{TAU_REW:g}_ts{TAU_STEP:g}"
              f"_lag{PRE_LAG_FRAMES}_iti{int(ZERO_ITI)}"
              f"{'' if SUBTRACT_POST_BASELINE else '_nobl'}")
...
print(f"pre_lag={PRE_LAG_FRAMES} frame(s), zero_iti={ZERO_ITI}, "
      f"subtract_post_baseline={SUBTRACT_POST_BASELINE}")
...
for mi in range(len(mice)):
    ...
    for sii in range(len(session_inds)):
        try:
            ...
            if SUBTRACT_POST_BASELINE:
                h_bar = ema_causal(df_full, tau_bl_frames, axis=-1,
                                   init=df_full[:, 0])
                dev = df_full - h_bar
            else:
                # Skip baseline entirely; use raw fluorescence as the
                # post-neuron term in the eligibility product.
                dev = df_full

            # Warm-start each baseline at the session-mean event rate so the
            # RPE lives at its asymptotic value from t=0 (approximates the
            # paper's cross-session carry-over of \bar r).
            reward_baseline = ema_causal(
                reward_vector.astype(float), tau_rew_frames,
                init=float(reward_vector.mean()))
            step_baseline = ema_causal(
                step_vector.astype(float), tau_step_frames,
                init=float(step_vector.mean()))
            ...
            result = {
                ...
                'trial_start_vector': trial_start_vector.astype(np.uint8),
...
OVERRIDE_TAU_STEP = 360.0   # seconds; e.g., set to the sweep optimum
...
if OVERRIDE_TAU_REW is not None or OVERRIDE_TAU_STEP is not None:
    ...
    for s in all_results:
        ...
        if OVERRIDE_TAU_REW is not None:
            rew_bl = ema_causal(rv, OVERRIDE_TAU_REW / dt,
                                init=float(rv.mean()))
            rew_rpe = rv - rew_bl
        ...
        if OVERRIDE_TAU_STEP is not None:
            step_bl = ema_causal(sv, OVERRIDE_TAU_STEP / dt,
                                 init=float(sv.mean()))
            step_rpe = sv - step_bl
...
# Ad-hoc: show RT panel as port speed = RT_INVERT_REF - RT, with x-label
# 'Port speed'.  Only affects Cell 9 (binned plot).
RT_TO_PORT_SPEED = True
RT_INVERT_REF = 10.0


def _maybe_invert_rt(bname, vals, label):
    """If this is the RT row and the flag is on, return (port_speed, label)."""
    if RT_TO_PORT_SPEED and bname == 'RT':
        return RT_INVERT_REF - np.asarray(vals, dtype=float), 'Port speed'
    return vals, label
...
for bi, (bname, blabel) in enumerate(zip(beh_names, beh_labels)):
    ax = axes[bi]
    # Apply the RT -> port speed swap locally (label change is per-panel).
    bvar_label = blabel
    if RT_TO_PORT_SPEED and bname == 'RT':
        bvar_label = 'Port speed'
    all_beh_z = []
    all_slope_z = []
    for s in all_results:
        bvar = get_beh(s, bname)
        if RT_TO_PORT_SPEED and bname == 'RT':
            bvar = RT_INVERT_REF - np.asarray(bvar, dtype=float)
        slope = s['hi_with_int']
        if TRIAL_CUTOFF is not None:
    ...
    ax.set_xlabel(f'{bvar_label} (within-session z)')
...
# Regression method: 'lasso' (default), 'ridge', or 'pinv' (unregularized OLS).
# - 'lasso' picks alpha via GroupKFold CV; coefficients are sparse.
# - 'ridge' picks alpha via GroupKFold CV over a log-spaced grid; dense coefs,
#   fits even when lasso shrinks everything to zero.
# - 'pinv' is np.linalg.pinv-style OLS; no penalty, no alpha selection.
...
# Grouped CV (precomputed splits since LassoCV/RidgeCV don't accept groups).
n_splits = min(LASSO_CV_FOLDS, len(unique_sessions))
gkf = GroupKFold(n_splits=n_splits)
cv_splits = list(gkf.split(X, y, groups))


def _refit(X_fit, y_fit, method, alpha=None):
    if method == 'lasso':
        m = Lasso(alpha=alpha, max_iter=20000, fit_intercept=False)
    elif method == 'ridge':
        m = Ridge(alpha=alpha, fit_intercept=False)
    elif method == 'pinv':
        m = LinearRegression(fit_intercept=False)
    else:
        raise ValueError(f"Unknown REGRESSION_TYPE: {method}")
    m.fit(X_fit, y_fit)
    return m


if REGRESSION_TYPE == 'lasso':
    _model_cv = LassoCV(cv=cv_splits, alphas=None, max_iter=20000, n_jobs=-1,
                        fit_intercept=False)
    _model_cv.fit(X, y)
    chosen_alpha = _model_cv.alpha_
    chosen_coef = _model_cv.coef_
    in_sample_r2 = float(_model_cv.score(X, y))
elif REGRESSION_TYPE == 'ridge':
    _alpha_grid = np.logspace(-4, 3, 40)
    _model_cv = RidgeCV(alphas=_alpha_grid, cv=cv_splits, fit_intercept=False)
    _model_cv.fit(X, y)
    chosen_alpha = float(_model_cv.alpha_)
    chosen_coef = _model_cv.coef_
    in_sample_r2 = float(_model_cv.score(X, y))
elif REGRESSION_TYPE == 'pinv':
    _model_cv = LinearRegression(fit_intercept=False)
    _model_cv.fit(X, y)
    chosen_alpha = None
    chosen_coef = _model_cv.coef_
    in_sample_r2 = float(_model_cv.score(X, y))
...
# Held-out cross-validated predictions: for each fold, refit and predict.
y_pred_cv = np.full_like(y, np.nan, dtype=float)
for tr_idx, te_idx in cv_splits:
    fold_model = _refit(X[tr_idx], y[tr_idx], REGRESSION_TYPE,
                        alpha=chosen_alpha)
...
_alpha_str = f"{chosen_alpha:.4g}" if chosen_alpha is not None else "(n/a)"
print(f"Method:             {REGRESSION_TYPE}")
print(f"Selected alpha:     {_alpha_str}")
print(f"In-sample R^2:      {in_sample_r2:.4f}")
print(f"CV (held-out) R^2:  {cv_r2:.4f}")
print(f"CV Pearson r:       {cv_r:.4f}  (p = {cv_p:.3e})")

# ---- Coefficient table ----
print(f"\n{REGRESSION_TYPE} coefficients (sorted by |coef|):")
for name, c in sorted(zip(beh_labels, chosen_coef),
...
for bi in range(LASSO_N_BOOTSTRAPS):
    ...
    _m = _refit(X_b, y_b, REGRESSION_TYPE, alpha=chosen_alpha)
    boot_coefs[bi] = _m.coef_

# Two stability metrics.  selection_freq is meaningful for lasso (fraction of
# resamples with non-zero coef).  sign_consistency is meaningful for all three
# (fraction of resamples with same sign as the fit_coef) and is the metric
# shown in the middle plot panel for ridge / pinv.
selection_freq = np.mean(np.abs(boot_coefs) > 1e-8, axis=0)
sign_consistency = np.array([
    np.mean(np.sign(boot_coefs[:, k]) == np.sign(chosen_coef[k]))
    if chosen_coef[k] != 0 else np.nan
    for k in range(len(chosen_coef))
])
...
order = np.argsort(-np.abs(chosen_coef))
for bi in order:
    print(f"  {beh_labels[bi]:25s} "
          f"{chosen_coef[bi]:+10.4f} {boot_mean[bi]:+10.4f} "
...
ax = axes[0]
order = np.argsort(-np.abs(chosen_coef))
colors = ['#3498db' if c > 0 else '#e74c3c' for c in chosen_coef[order]]
ax.bar(range(len(order)), chosen_coef[order], color=colors, edgecolor='k')
for xi, bi in enumerate(order):
    # Clamp at 0: with sparse-selecting Lasso, boot_mean can drift outside
    # [boot_lo, boot_hi] when the CI edges are exactly 0 but the mean is
    # slightly non-zero.
    _yerr_lo = max(0.0, float(boot_mean[bi] - boot_lo[bi]))
    _yerr_hi = max(0.0, float(boot_hi[bi]  - boot_mean[bi]))
...
_alpha_str_title = (f"alpha={chosen_alpha:.3g}" if chosen_alpha is not None
                    else "no penalty")
ax.set_title(f'{REGRESSION_TYPE} coefficients ({_alpha_str_title}, '
             f'in-sample R²={in_sample_r2:.3f})',
             fontsize=12, fontweight='bold')

ax = axes[1]
# For lasso, "selection frequency" is the natural stability metric.  For
# ridge / pinv, all coefficients are always non-zero, so sign_consistency
# is more informative.
if REGRESSION_TYPE == 'lasso':
    _stab = selection_freq
    _stab_label = 'Selection frequency'
else:
    _stab = np.where(np.isfinite(sign_consistency), sign_consistency, 0.0)
    _stab_label = 'Sign consistency'
ax.bar(range(len(order)), _stab[order], color='#7f8c8d', edgecolor='k')
ax.axhline(0.5, color='k', ls='--', alpha=0.5)
ax.set_xticks(range(len(order)))
ax.set_xticklabels([beh_labels[i] for i in order], rotation=30, ha='right')
ax.set_ylabel(_stab_label)
ax.set_ylim(0, 1)
ax.set_title(f'Bootstrap {_stab_label.lower()} '
             f'(n={LASSO_N_BOOTSTRAPS}, GroupKFold by session)',
             fontsize=12, fontweight='bold')
...
fig.suptitle(
    ...
    fontsize=12, fontweight='bold', y=1.02)
plt.tight_layout()
_method_tag = f'_{REGRESSION_TYPE}' if REGRESSION_TYPE != 'lasso' else ''
_p = os.path.join(RESULTS_DIR,
        f'fig_continuous_eligibility_lasso{CONFIG_TAG}{_override_tag}{_lasso_cutoff_tag}{_method_tag}.png')
plt.savefig(_p, dpi=150, bbox_inches='tight')
_save_panel(_p)
plt.show()
