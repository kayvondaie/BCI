...
fit_type      = 'ridge_pos'     #ridge, pinv
alpha         =  100        #only used for ridge
num_bins      =  60        # number of bins to calculate correlations
tau_elig      =  2
shuffle       =  0
plotting      =  1
for mi in range(len(mice)):
    ...
    #for sii in range(si,si+1):
        try:
            ...
            def get_indices_far_from_steps(tsta, steps, window=0):                
                tsta = np.asarray(tsta)
                steps = np.asarray(steps)
                
                # Create mask of timepoints that are too close to any step
                too_close = np.zeros_like(tsta, dtype=bool)
                for step in steps:
                    too_close |= np.abs(tsta - step) < window
            
                # Invert mask to get timepoints far from all steps
                safe_indices = np.where(~too_close)[0]
                return safe_indices
            ...
            for ti in range(trl):
                ...
                if len(steps) > 0:
                    indices_steps = get_indices_far_from_steps(tsta, steps, window = 1.0)
                    indices_steps = indices_steps[indices_steps < F.shape[0]]
                    k[:, ti] = np.nanmean(F[indices_steps, :, ti], axis=0)
                    
                # Rewards regressor
                rewards = data['reward_time'][ti]
                if len(rewards) > 0:
                    indices_rewards = get_indices_around_steps(tsta, rewards, pre=10, post=1)
            ...
            Xts_mod   = 0*Xts_T   @ behavior_features
    ...
    for i in range(len(HI)):
        hi[i] = hi[i] * weight[i]
    hi = np.asarray(hi)
    hi = np.nanmean(hi,axis = 0)
    sns.heatmap(hi*1000, annot=True, xticklabels=['rt', 'rt_rpe', 'hit', 'hit_rpe', 'miss', 'miss_rpe'], yticklabels=['step', 'reward', 'go cue'], cmap='coolwarm', center=0)
#%%
