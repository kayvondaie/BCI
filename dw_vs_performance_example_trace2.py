...
for gi in range(stimDist.shape[1]):
    ...
    for targ in targets:
        for nt in nontargs:
            ...
            if (AMP[1][nt, gi] > amp_thresh) and (coact < 0) and (dw < 0) and (np.abs(dwt) < .4 and (wt > .5)):
...
def plot_average_candidates(candidates, F, FAVG, hit, tsta, dt_si, ksize=3):
    ...
    ax = axes[1, 0]
    ax.plot(tstim, pre_bef_avg, 'k:', lw=2, label='Bef.')
    ax.plot(tstim, pre_aft_avg, 'k', lw=2, label='Aft.')
    ...
    ax = axes[1, 1]
    ax.plot(tstim, post_bef_avg, 'k:', lw=2, label='Pre')
    ax.plot(tstim, post_aft_avg, 'k', lw=2, label='Post')
    ax.axvline(0, color='k', ls=':')
    ...
    plt.suptitle(f"Average over {len(candidates)} pairs")
    plt.tight_layout()
    plt.show()
tsta = tsta[:F.shape[0]]
plot_average_candidates(candidates, F, FAVG, hit, tsta, dt_si, ksize=11)
#%%
...
plt.plot(trial_bins[1:-1], zscore(rt_bin[1:-1]), 'k')
