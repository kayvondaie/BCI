...
def compute_amp_from_photostim_artifact_free(mouse, data, folder):
    """
    Artifact-free version of compute_amp_from_photostim.

    Photostim now occurs between frames, so there is no whole-frame
    light-leakage artifact to detect or mask. We therefore:
      - skip artifact detection / NaN-masking / interpolation entirely
      - take the photostim window from stim_params (`time` and `total_duration`)
        rather than inferring it from a 0.5-amplitude blob in the trace

    Also uses an asymmetric dF/F that is more robust to dim cells:
      - numerator   : per-(cell, group) 3-frame baseline (preserves
                      group-specific pre-stim drift correction)
      - denominator : per-cell baseline averaged across stim groups
                      (stable; no 1/F0 blow-up when an individual group's
                      pre-stim happens to be near zero)

    Parameters
    ----------
    mouse : str
        Mouse ID (e.g., 'BCI103') — affects post-stim window length.
    data : dict
        Dictionary containing photostim data. Expects each epoch dict
        (data['photostim'], optionally data['photostim2']) to contain:
          - 'stimDist'   : (n_cells, n_groups) distance from stim site (pix)
          - 'favg_raw'   : (n_frames, n_cells, n_groups) trial-avg raw F
          - 'stim_params': dict with 'time' (per-frame time-from-stim, s)
                           and 'total_duration' (laser-on duration, s)
    folder : str
        Full path to suite2p folder containing siHeader.npy.
    ...
    -------
    AMP : list of np.ndarray
        List of AMP values for each epoch (each array shape (n_cells, n_groups)).
    stimDist : np.ndarray
        Distance from stim site for each cell (in microns), last epoch's.
    """
    AMP = []
    ...
    n_epochs = 2 if 'photostim2' in data else 1
    for epoch_i in range(n_epochs):
        ep = data['photostim'] if epoch_i == 0 else data['photostim2']
        stimDist = ep['stimDist'] * umPerPix
        favg_raw = ep['favg_raw']
        stim_params = ep['stim_params']

        # Stim window from stim_params: start = frame where time==0,
        # end = first frame where time >= total_duration.
        time_arr = np.asarray(stim_params['time']).ravel()
        total_duration = float(np.asarray(stim_params['total_duration']).ravel()[0])
        start = int(np.where(np.isclose(time_arr, 0.0))[0][0])
        end_hits = np.where(time_arr >= total_duration)[0]
        end = int(end_hits[0]) if len(end_hits) > 0 else start + 16

        # Asymmetric dF/F: per-group baseline subtract, per-cell across-group divide.
        bl_per_group = np.nanmean(favg_raw[0:3, :, :], axis=0)        # (n_cells, n_groups)
        bl_per_cell  = np.nanmean(bl_per_group, axis=-1)              # (n_cells,)
        favg = (favg_raw - bl_per_group[None, :, :]) / bl_per_cell[None, :, None]

        before = int(np.floor(0.2 / dt_si))

        # Post = exactly the stim window (no artifact to skip).
        # Pre  = 200 ms baseline immediately preceding stim onset.
        pre  = (start - before, start)
        post = (start, end)

        amp = (np.nanmean(favg[post[0]:post[1], :, :], axis=0)
               - np.nanmean(favg[pre[0]:pre[1], :, :], axis=0))
        AMP.append(amp)
...
def compute_binned_behaviors_and_pairwise(
    *,
    ...
    Inputs:
      ...
      - pairwise_mode: one of the following:
            'dot_prod'        — Hebbian: CC[a,b] = sum_t r_a(t) * r_b(t)
            'noise_corr'      — Hebbian (normalized): CC = corrcoef, trial-to-trial noise correlations
            'dot_prod_no_mean'— Hebbian (centered): CC = (r-mean)@(r-mean).T, removes signal correlation
            'dot_prod_z'      — Hebbian (std-normalized): CC = (r/std)@(r/std).T, scales by variance
            'dot_prod_dev'    — Hebbian with go_cue deviation from early-trial baseline
            'lag_to_late'     — Cross-epoch: all epochs dotted against late-epoch activity
            'lag_to_rew'      — Cross-epoch: all epochs dotted against reward-epoch activity
            'pre_only'        — Non-Hebbian: CC[a,b] = sum_t r_a(t), depends only on presynaptic neuron
            'post_only'       — Non-Hebbian: CC[a,b] = sum_t r_b(t), depends only on postsynaptic neuron
    ...
    for i in range(nb):
        ...
        elif pairwise_mode == "pre_only":
            # r_i only: CC[a,b] = sum_t r_a(t), independent of neuron b
            ones_t = np.ones_like(krewards[:, ind])
            CCrew[:, :, i]  = krewards[:, ind] @ ones_t.T
            CCstep[:, :, i] = kstep[:, ind]    @ ones_t.T
            CCts[:, :, i]   = k[:, ind]        @ ones_t.T
            CCpre[:, :, i]  = kpre[:, ind]     @ ones_t.T

        elif pairwise_mode == "post_only":
            # r_j only: CC[a,b] = sum_t r_b(t), independent of neuron a
            ones_t = np.ones_like(krewards[:, ind])
            CCrew[:, :, i]  = ones_t @ krewards[:, ind].T
            CCstep[:, :, i] = ones_t @ kstep[:, ind].T
            CCts[:, :, i]   = ones_t @ k[:, ind].T
            CCpre[:, :, i]  = ones_t @ kpre[:, ind].T
...
def _compute_pairwise_single_trial(F_trial, mode, centered_dot=None, zscore_mat=None):
    ...
    if mode == 'dot_prod':
        ...
        # r_i only: CC[a,b] = sum_t r_a(t), independent of neuron b
        ones_t = np.ones_like(F_trial)
        return F_trial.T @ ones_t

    elif mode == 'post_only':
        # r_j only: CC[a,b] = sum_t r_b(t), independent of neuron a
        ones_t = np.ones_like(F_trial)
        return ones_t.T @ F_trial
