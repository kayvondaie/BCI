...
# Per-transition classification: slope of RT across post-switch trials
# Negative slope = recovering, positive = deteriorating
post_rt = rt_aligned[:, PRE:PRE + POST]   # (n_transitions, POST)
...
mask_better = post_rt_slope < 0   # RT declining post-switch (recovering)
mask_worse = post_rt_slope > 0    # RT climbing post-switch (deteriorating)

for dir_label, dir_mask, dir_suffix in [('Recovering (post RT slope < 0)', mask_better, 'better'),
                                         ('Deteriorating (post RT slope > 0)', mask_worse, 'worse'),
                                         ]:
