...
#%% ============================================================================
# CELL 13: CN robustness diagnostics — is the post-switch CN increase real
# without depending on per-transition std normalization?
# ============================================================================
# Tests whether the z-scored CN increase after threshold change holds up under
# weaker normalizations:
#   (1) Delta only (subtract pre-mean, no std division)
#   (2) Per-transition (post − pre) distribution + binomial sign test
#   (3) Robust scaling (MAD instead of std; std with floor)
#   (4) Pre-std vs effect-size scatter — see if a few low-pre-std transitions
#       drive the apparent z-score effect.
#
# Uses cn_aligned (n_switches, PRE+POST) and thr_direction from CELL 8.
# Restricts to threshold INCREASES (the case the user finds robust).

from scipy.stats import binomtest

cn_inc = cn_aligned[thr_direction == 1]
n_inc  = cn_inc.shape[0]
trial_ax = np.arange(-PRE, POST)

pre_mean_inc = np.nanmean(cn_inc[:, :PRE], axis=1, keepdims=True)
pre_std_inc  = np.nanstd (cn_inc[:, :PRE], axis=1, keepdims=True)

# Robust scale: MAD; floored std
pre_mad_inc = np.nanmedian(np.abs(cn_inc[:, :PRE] -
                                   np.nanmedian(cn_inc[:, :PRE], axis=1, keepdims=True)),
                           axis=1, keepdims=True) * 1.4826  # ~unbiased for normal
std_floor   = np.nanpercentile(pre_std_inc.squeeze(), 25)   # 25th percentile floor
pre_std_floored = np.maximum(pre_std_inc, std_floor)

# Full-window std (pre + post), per transition — more stable than pre-only std
full_std_inc = np.nanstd(cn_inc, axis=1, keepdims=True)

# Versions of the trace
cn_delta = cn_inc - pre_mean_inc
cn_z     = (cn_inc - pre_mean_inc) / np.where(pre_std_inc > 1e-9, pre_std_inc, np.nan)
cn_zfl   = (cn_inc - pre_mean_inc) / pre_std_floored
cn_zmad  = (cn_inc - pre_mean_inc) / np.where(pre_mad_inc > 1e-9, pre_mad_inc, np.nan)
cn_zfull = (cn_inc - pre_mean_inc) / np.where(full_std_inc > 1e-9, full_std_inc, np.nan)
# (post - pre)/pre  =  fractional change from baseline (per timepoint)
cn_relchg = (cn_inc - pre_mean_inc) / np.where(np.abs(pre_mean_inc) > 1e-9,
                                                pre_mean_inc, np.nan)

# Per-transition (post - pre) means
post_minus_pre = (np.nanmean(cn_inc[:, PRE:], axis=1)
                  - np.nanmean(cn_inc[:, :PRE], axis=1))
ok_pp = np.isfinite(post_minus_pre)
post_pre_ok = post_minus_pre[ok_pp]
n_pos = int(np.sum(post_pre_ok > 0))
n_total = int(len(post_pre_ok))
binom = binomtest(n_pos, n_total, p=0.5, alternative='two-sided')
median_dpp = float(np.median(post_pre_ok))
mean_dpp   = float(np.mean(post_pre_ok))

# Per-transition (post - pre)/pre  (relative change)
pre_mean_1d = pre_mean_inc.squeeze()
post_mean_1d = np.nanmean(cn_inc[:, PRE:], axis=1)
rel_change = (post_mean_1d - pre_mean_1d) / np.where(np.abs(pre_mean_1d) > 1e-9,
                                                      pre_mean_1d, np.nan)
ok_rc = np.isfinite(rel_change)
rc_ok = rel_change[ok_rc]
n_pos_rc = int(np.sum(rc_ok > 0))
n_total_rc = int(len(rc_ok))
binom_rc = binomtest(n_pos_rc, n_total_rc, p=0.5, alternative='two-sided')
median_rc = float(np.median(rc_ok))
mean_rc   = float(np.mean(rc_ok))

# Effect size per transition (post mean - pre mean), used in scatter
eff_size = post_minus_pre

print(f"\n{'='*78}")
print(f"CELL 13: CN robustness diagnostics  (threshold INCREASES, n={n_inc})")
print(f"{'='*78}")
print(f"  Per-transition (post - pre):")
print(f"    mean   = {mean_dpp:+.4f}")
print(f"    median = {median_dpp:+.4f}")
print(f"    {n_pos}/{n_total} ({100*n_pos/n_total:.1f}%) transitions had post > pre")
print(f"    Binomial sign test (two-sided): p = {binom.pvalue:.3g}")
print(f"  Per-transition (post - pre)/pre (relative change):")
print(f"    mean   = {mean_rc*100:+.2f}%")
print(f"    median = {median_rc*100:+.2f}%")
print(f"    {n_pos_rc}/{n_total_rc} ({100*n_pos_rc/n_total_rc:.1f}%) transitions had post > pre")
print(f"    Binomial sign test (two-sided): p = {binom_rc.pvalue:.3g}")
print(f"  Pre-std distribution: min={np.nanmin(pre_std_inc):.4f}, "
      f"25th={std_floor:.4f}, median={np.nanmedian(pre_std_inc):.4f}, "
      f"max={np.nanmax(pre_std_inc):.4f}")

def _mean_sem_loc(arr, axis=0):
    m = np.nanmean(arr, axis=axis)
    n = np.sum(np.isfinite(arr), axis=axis)
    s = np.nanstd(arr, axis=axis) / np.sqrt(np.clip(n, 1, None))
    return m, s
...
# --- Plot ---
fig13, axes = plt.subplots(2, 3, figsize=(15, 8))

# (1) Delta-only trace alongside z-scored
ax = axes[0, 0]
m_d, s_d = _mean_sem_loc(cn_delta)
m_z, s_z = _mean_sem_loc(cn_z)
ax.plot(trial_ax, m_d, 'k', linewidth=1.4, label='Δ CN (raw units)')
ax.fill_between(trial_ax, m_d - s_d, m_d + s_d, color='k', alpha=0.18)
ax2 = ax.twinx()
ax2.plot(trial_ax, m_z, 'C3', linewidth=1.4, label='z-scored')
ax2.fill_between(trial_ax, m_z - s_z, m_z + s_z, color='C3', alpha=0.15)
ax.axvline(0, color='gray', linestyle='--', linewidth=0.8)
ax.axhline(0, color='gray', linestyle=':', linewidth=0.5)
ax.set_xlabel('Trials from threshold change')
ax.set_ylabel('Δ CN (mean-subtracted)')
ax2.set_ylabel('z-scored CN', color='C3')
ax2.tick_params(axis='y', labelcolor='C3')
ax.set_title(f'(1) Delta vs z-score (n={n_inc})', fontsize=11)
...
# (2) Histogram of per-transition (post - pre) with sign test
ax = axes[0, 1]
n_bins = max(15, int(np.sqrt(len(post_pre_ok))))
ax.hist(post_pre_ok, bins=n_bins, color='#bbbbbb', edgecolor='k', linewidth=0.5)
ax.axvline(0, color='k', linestyle=':', linewidth=1.2)
ax.axvline(median_dpp, color='C3', linewidth=2,
           label=f'median={median_dpp:+.3f}')
ax.set_xlabel('Per-transition (post − pre) CN')
ax.set_ylabel('# transitions')
ax.set_title(f'(2) {n_pos}/{n_total} > 0  '
             f'(binomial p={binom.pvalue:.3g})', fontsize=11)
ax.legend(loc='best', fontsize=9, frameon=False)
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)

# (3) Robust scaling — pre-std vs floored vs MAD vs full-window std
ax = axes[1, 0]
for arr, color, lbl in [
    (cn_z,    'C3', 'z (pre std)'),
    (cn_zfl,  'C0', 'z (pre std, floored 25th pct)'),
    (cn_zmad, 'C2', 'z (pre MAD)'),
    (cn_zfull,'C4', 'z (full pre+post std)'),
]:
    m, s = _mean_sem_loc(arr)
    ax.plot(trial_ax, m, color=color, linewidth=1.4, label=lbl)
    ax.fill_between(trial_ax, m - s, m + s, color=color, alpha=0.12)
ax.axvline(0, color='gray', linestyle='--', linewidth=0.8)
ax.axhline(0, color='gray', linestyle=':', linewidth=0.5)
ax.set_xlabel('Trials from threshold change')
ax.set_ylabel('Normalized CN')
ax.set_title('(3) Different std/MAD denominators', fontsize=11)
ax.legend(loc='best', fontsize=7, frameon=False)
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)

# (4) pre_std vs (post - pre) per transition
ax = axes[1, 1]
xs = pre_std_inc.squeeze()
ys = eff_size
ok = np.isfinite(xs) & np.isfinite(ys)
ax.scatter(xs[ok], ys[ok], s=15, c='k', alpha=0.5, edgecolors='none')
ax.axhline(0, color='k', linestyle=':', linewidth=0.8)
# Linear fit just for visual reference
if np.sum(ok) >= 3:
    p = np.polyfit(xs[ok], ys[ok], 1)
    xfit = np.linspace(np.nanmin(xs[ok]), np.nanmax(xs[ok]), 50)
    ax.plot(xfit, p[0] * xfit + p[1], 'C3', linewidth=1.2, alpha=0.7,
            label=f'fit slope={p[0]:+.3f}')
    ax.legend(loc='best', fontsize=9, frameon=False)
ax.set_xlabel('Pre-switch std (raw CN units)')
ax.set_ylabel('Per-transition (post − pre) CN')
ax.set_title(f'(4) Pre-std vs effect size (n={int(np.sum(ok))})',
             fontsize=11)
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)

# (5) Mean trace of (cn - pre)/pre  (relative change vs trial offset)
ax = axes[0, 2]
m, s = _mean_sem_loc(cn_relchg)
ax.plot(trial_ax, m * 100, color='k', linewidth=1.4)
ax.fill_between(trial_ax, (m - s) * 100, (m + s) * 100, color='k', alpha=0.18)
ax.axvline(0, color='gray', linestyle='--', linewidth=0.8)
ax.axhline(0, color='gray', linestyle=':', linewidth=0.5)
ax.set_xlabel('Trials from threshold change')
ax.set_ylabel('(CN − pre) / pre (%)')
ax.set_title('(5) Relative change from baseline (%)', fontsize=11)
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)

# (6) Per-transition (post - pre)/pre histogram + binomial sign test
ax = axes[1, 2]
n_bins = max(15, int(np.sqrt(len(rc_ok))))
ax.hist(rc_ok * 100, bins=n_bins, color='#bbbbbb', edgecolor='k', linewidth=0.5)
ax.axvline(0, color='k', linestyle=':', linewidth=1.2)
ax.axvline(median_rc * 100, color='C3', linewidth=2,
           label=f'median={median_rc*100:+.1f}%')
ax.set_xlabel('Per-transition (post − pre) / pre  (%)')
ax.set_ylabel('# transitions')
ax.set_title(f'(6) {n_pos_rc}/{n_total_rc} > 0  '
             f'(binomial p={binom_rc.pvalue:.3g})', fontsize=11)
ax.legend(loc='best', fontsize=9, frameon=False)
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)

fig13.suptitle('CN robustness diagnostics — does the post-switch increase '
               'survive without per-transition std normalization?',
               fontsize=12, fontweight='bold')
plt.tight_layout()
fname13 = 'cn_robustness_diagnostics_inc'
fig13.savefig(os.path.join(PANEL_DIR, f'{fname13}.png'), dpi=300)
fig13.savefig(os.path.join(PANEL_DIR, f'{fname13}.svg'))
plt.show()
print(f"Saved {fname13}")
