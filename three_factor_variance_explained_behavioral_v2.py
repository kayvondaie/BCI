...
# ============================================================================
"""
Variant of three_factor_variance_explained_behavioral.py with 5 CC modes:
  1. dot_prod       — sum_t pre(t) * post(t)
  2. dev2           — sum_t pre(t) * (post(t) - baseline_post)
  3. pre_dev_only   — sum_t (pre(t) - baseline_pre) * post(t)
  4. pre_dev        — sum_t (pre(t) - baseline_pre) * (post(t) - baseline_post)
  5. phi_prime_dev2 — sum_t pre(t) * (post(t) - baseline_post) * 1(post > 20th pctl)
...
all_sessions = []
print(f"Data source: {'LOCAL' if USE_LOCAL_DATA else 'SHARED'} ({DATA_BASE})")
        ...
        try:
            ...
            for ep in EPOCH_ORDER:
                baseline_post_mean_ep[ep] = np.nanmean(
                    epoch_act[ep][:, bl_trials], axis=1)  # (n_neurons,)
            ...
            for ep in EPOCH_ORDER:
                pctl20_post_ep[ep] = np.percentile(
                    epoch_act[ep], 20, axis=1)  # (n_neurons,)
            ...
            for cc_mode in CC_MODES:
                for ep in EPOCH_ORDER:
                    ...
                    # Compute per-trial CC depending on mode
                    if cc_mode == 'dot_prod':
                        cc_trial = r_pre * r_post

                    elif cc_mode == 'dev2':
                        post_dev = r_post - baseline_post_mean_ep[ep][all_nt, np.newaxis]
                        cc_trial = r_pre * post_dev

                    elif cc_mode == 'pre_dev_only':
                        pre_dev = r_pre - baseline_pre_pair[ep][:, np.newaxis]
                        cc_trial = pre_dev * r_post

                    elif cc_mode == 'pre_dev':
                        pre_dev = r_pre - baseline_pre_pair[ep][:, np.newaxis]
                        post_dev = r_post - baseline_post_mean_ep[ep][all_nt, np.newaxis]
                        cc_trial = pre_dev * post_dev

                    elif cc_mode == 'phi_prime_dev2':
                        post_dev = r_post - baseline_post_mean_ep[ep][all_nt, np.newaxis]
                        gate = (r_post > pctl20_post_ep[ep][all_nt, np.newaxis]).astype(float)
                        cc_trial = r_pre * post_dev * gate
...
# CELL 4: Save
# ============================================================================
np.save(os.path.join(RESULTS_DIR, 'three_factor_variance_explained_behavioral_v2.npy'),
...
all_sessions = np.load(
    os.path.join(RESULTS_DIR, 'three_factor_variance_explained_behavioral_v2.npy'),
...
fig, axes = plt.subplots(n_epochs, n_beh * n_modes, figsize=(50, 12))
    ...
    for mi, cc_mode in enumerate(CC_MODES):
        for bi, (bname, blabel) in enumerate(zip(BEH_NAMES, BEH_LABELS)):
            ...
            colors = {
                ...
                'pre_dev_only': '#e67e22',
                'pre_dev': '#8e44ad',
                'phi_prime_dev2': '#27ae60',
            }
            color = colors.get(cc_mode, 'steelblue')
...
plt.tight_layout()
plt.savefig(os.path.join(RESULTS_DIR, 'three_factor_behavioral_v2_summary.png'),
...
Side-by-side heatmaps for each CC mode, plus fitted-beta comparison.
"""
fig, axes = plt.subplots(1, n_modes + 1, figsize=(6 * n_modes + 3, 4),
                         gridspec_kw={'width_ratios': [1] * n_modes + [0.4]})
...
for mi, cc_mode in enumerate(CC_MODES):
    ...
    ax.set_xticklabels(BEH_LABELS, rotation=30, ha='right', fontsize=8)
    ax.set_yticks(range(n_epochs))
    ax.set_yticklabels(EPOCH_ORDER)
    ax.set_title(f'{cc_mode}', fontweight='bold', fontsize=9)
    plt.colorbar(im, ax=ax, shrink=0.8, label='Mean ρ')

# Fitted-beta comparison panel
ax = axes[n_modes]
...
for ei in range(n_epochs):
    for mi in range(n_modes):
        ...
        if np.isfinite(val):
            ax.text(mi, ei, f'{val:+.4f}', ha='center', va='center', fontsize=7)
ax.set_xticks(range(n_modes))
ax.set_xticklabels(CC_MODES, rotation=30, ha='right', fontsize=7)
ax.set_yticks(range(n_epochs))
ax.set_yticklabels(EPOCH_ORDER)
ax.set_title('Fitted β\n(CV test r)', fontweight='bold', fontsize=9)
plt.colorbar(im2, ax=ax, shrink=0.8, label='Mean test r')

fig.suptitle('Behavioral weighting vs fitted betas (5 CC modes)\n'
             'dW = Σ_w behavior_w × CC_w',
             fontsize=14, fontweight='bold', y=1.05)
plt.tight_layout()
plt.savefig(os.path.join(RESULTS_DIR, 'three_factor_behavioral_v2_heatmap.png'),
            dpi=200, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(RESULTS_DIR, 'three_factor_pooled_v2_focused.png'),
            dpi=200, bbox_inches='tight')
plt.show()

#%% ============================================================================
# CELL 9: Pooled binned plots — RPE × CC per CC mode (pre epoch)
# ============================================================================
"""
One subplot per CC mode. Each shows the zero-parameter RPE-weighted prediction
(Σ_w RPE_w * CC_w) vs actual dW, pooled across sessions with per-session z-scoring.
"""
from scipy.stats import pearsonr

CC_MODES_PLOT = ['dot_prod', 'dev2', 'pre_dev_only', 'pre_dev', 'phi_prime_dev2']
mode_colors = {
    'dot_prod': '#95a5a6',
    'dev2': '#2c3e50',
    'pre_dev_only': '#e67e22',
...
fig, axes = plt.subplots(1, len(CC_MODES_PLOT), figsize=(4 * len(CC_MODES_PLOT), 4))

for mi, cc_mode in enumerate(CC_MODES_PLOT):
    ax = axes[mi]

    # Pool pairs across sessions for this CC mode, pre epoch, RPE weighting
    Y_pool_m = []
    X_pool_m = []
    key_pred = f'{cc_mode}_pre_RPE_pred'

    for s in all_sessions:
        y = s['Y']
        x = s.get(key_pred, None)
        if x is None:
            continue
        mask = np.isfinite(y) & np.isfinite(x)
        if np.sum(mask) < 5:
            continue
        y_s, x_s = y[mask], x[mask]

        # Z-score within session
        def _z(v):
            mu, sig = np.mean(v), np.std(v)
            return (v - mu) / sig if sig > 0 else v - mu
        Y_pool_m.append(_z(y_s))
        X_pool_m.append(_z(x_s))

    if len(Y_pool_m) == 0:
        ax.set_title(f'{cc_mode}\nno data')
        continue

    Y_pool_m = np.concatenate(Y_pool_m)
    X_pool_m = np.concatenate(X_pool_m)

    r_val, p_val = spearmanr(X_pool_m, Y_pool_m)

    binned_plot(ax, X_pool_m, Y_pool_m, n_bins=8, color=mode_colors[cc_mode])
    ax.axhline(0, color='k', ls='-', alpha=0.2)
    ax.set_xlabel('Σ RPE × CC (pre)')
    if mi == 0:
        ax.set_ylabel('ΔW (z)')
    ax.set_title(f'{cc_mode}\nρ = {r_val:.4f}, p = {p_val:.2e}',
                 fontweight='bold', fontsize=10)

fig.suptitle(f'Pooled RPE × CC vs ΔW by CC mode (pre epoch, n={len(all_sessions)} sessions)',
             fontsize=14, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(RESULTS_DIR, 'three_factor_pooled_v2_by_cc_mode.png'),
            dpi=200, bbox_inches='tight')
plt.show()

#%% ============================================================================
# CELL 10: Pooled binned plots — equal weight per session
# ============================================================================
"""
Same as Cell 9 but each session contributes equally: subsample to the minimum
pair count across sessions so no session dominates the pooled statistics.
"""
...
for mi, cc_mode in enumerate(CC_MODES_PLOT):
    ...
    # Collect per-session z-scored arrays
    sess_y = []
    sess_x = []
    for s in all_sessions:
        y = s['Y']
        ...
        def _z(v):
            mu, sig = np.mean(v), np.std(v)
            return (v - mu) / sig if sig > 0 else v - mu
        sess_y.append(_z(y_s))
        sess_x.append(_z(x_s))

    if len(sess_y) == 0:
        ax.set_title(f'{cc_mode}\nno data')
        continue

    # Subsample each session to min pair count
    min_pairs = min(len(sy) for sy in sess_y)
    Y_eq = []
    X_eq = []
    for sy, sx in zip(sess_y, sess_x):
        idx = rng.choice(len(sy), size=min_pairs, replace=False)
        Y_eq.append(sy[idx])
        X_eq.append(sx[idx])
    Y_eq = np.concatenate(Y_eq)
    X_eq = np.concatenate(X_eq)

    r_val, p_val = spearmanr(X_eq, Y_eq)

    binned_plot(ax, X_eq, Y_eq, n_bins=8, color=mode_colors[cc_mode])
    ax.axhline(0, color='k', ls='-', alpha=0.2)
    ax.set_xlabel('Σ RPE × CC (pre)')
    if mi == 0:
        ax.set_ylabel('ΔW (z)')
...
n_sess = len(sess_y)
fig.suptitle(f'Pooled RPE × CC vs ΔW — equal weight per session\n'
             f'(pre epoch, {n_sess} sessions, {min_pairs} pairs/session)',
             fontsize=14, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(RESULTS_DIR, 'three_factor_pooled_v2_by_cc_mode_equal_wt.png'),
            dpi=200, bbox_inches='tight')
plt.show()
