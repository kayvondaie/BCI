...
for mi in range(len(mice)):
    ...
    pairwise_mode = 'post_only'  #dot_prod, noise_corr,dot_prod_no_mean, pre_only, post_only 
        ...
        try:
            ...
            for i in range(len(trial_bins)-1):
                ...
                if pairwise_mode == 'noise_corr':    
                    ...
                    CCrew[:,:,i] = np.dot(krewards[:,ind],np.ones(krewards[:,ind].T.shape))
                    CCstep[:,:,i] = np.dot(kstep[:,ind],np.ones(krewards[:,ind].T.shape))
                    CCts[:,:,i] = np.dot(k[:,ind],np.ones(krewards[:,ind].T.shape));
                    CCpre[:,:,i] = np.dot(kpre[:,ind],np.ones(krewards[:,ind].T.shape));
                elif pairwise_mode == 'pre_only':
                    CCrew[:,:,i] = np.dot(np.ones(krewards[:,ind].shape),krewards[:,ind].T)
                    CCstep[:,:,i] = np.dot(np.ones(kstep[:,ind].shape),kstep[:,ind].T)
                    CCts[:,:,i] = np.dot(np.ones(k[:,ind].shape),k[:,ind].T);
                    CCpre[:,:,i] = np.dot(np.ones(kpre[:,ind].shape),kpre[:,ind].T);
