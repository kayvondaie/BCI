...
tif_path = '//allen/aind/scratch/BCI/2p-raw/BCI93/091124/pophys/neuron1_newThresh_00001.tif'

# Read the TIF file
tif_data = tiff.imread(tif_path)

# Keep the first frame and every other frame
selected_frames = tif_data[::1]
...
# Base path and file naming pattern
base_path = '//allen/aind/scratch/BCI/2p-raw/BCI93/091124/pophys/'
base_name = 'neuron1_newThresh_'

# Number of TIF files to process
num_tifs = 3

# Generate TIF paths automatically
tif_paths = [os.path.join(base_path, f"{base_name}{i:05d}.tif") for i in range(1, num_tifs + 1)]

# List to store frames from all TIF files
all_frames = []
...
for tif_path in tif_paths:
    if os.path.exists(tif_path):  # Check if the file exists
        ...
        # Keep every frame (modify indexing if needed)
        selected_frames = tif_data[::1]
...
# Calculate the running average for smoothing
smoothed_frames = running_average(all_frames)
