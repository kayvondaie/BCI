...
for si in range(len(THR_WINDOW)):
    ...
    for i in range(len(ind)):
        a = y[ind[i]-pre:ind[i]+post]
        aa = yy[ind[i]-pre:ind[i]+post]
        a = a - np.nanmean(a[0:pre])
        a = medfilt(a, kernel_size=9)
...
AA_array = np.column_stack(AA)
B_array = np.column_stack(B)
d = np.nanmean(B_array[pre:pre+10],axis=0)
ind1 = np.where(d==1)[0];
ind2 = np.where(d!=1)[0]
plt.plot(np.nanmean(A_array[:,ind1],axis=1));plt.plot(np.nanmean(A_array[:,ind2],axis=1))
...
for i in range(1,len(RPE)):
    d_rpe = np.nanmean(RPE[i][0:]) - np.nanmean(RPE[i-1][10:40])
    dd.append(d_rpe)
plt.figure(figsize = (8,3))
plt.subplot(121)
plt.scatter(dd,CC_RPE[1:])
plt.subplot(122)
plt.scatter(dd,CC_RPE[1:])
plt.tight_layout()
pearsonr(dd,CC_RPE[1:])
...
plt.subplot(121)
pf.mean_bin_plot(X[inds[0],:],Y[0,:],5,1,1,'k')
plt.xlabel('Correlation with target neuron (early trials)')
plt.ylabel('Connection')

plt.subplot(122)
pf.mean_bin_plot(X[inds[-2],:],Y[0,:],5,1,1,'k')
...
for i in range(len(inds)):
    cc[i] = np.corrcoef((Y-Yo).flatten(),X[inds[i],:])[0,1]
    #cco[i] = np.corrcoef(Yo.flatten(),X[inds[i],:])[0,1]
plt.figure(figsize = (9,3))
plt.subplot(131)
plt.plot(trial_bins,cc,'k')
# plt.plot(trial_bins,cco,'k')
plt.ylabel('CC vs. $\Delta$ W correlation');
plt.xlabel('Trial #')
plt.subplot(132)
plt.plot(trial_bins,a,'k')
plt.title(data['mouse'] + '  ' + data['session'])
plt.ylabel('MRL coeffs');
plt.xlabel('Trial #')
...
# Plot the slope over trial bins
plt.subplot(133)
plt.plot(trial_bins, slope, 'k')
plt.ylabel('Regression slope (ΔW ~ corr)')
plt.xlabel('Trial #')


plt.suptitle(data['mouse'] + '  ' + data['session'], fontsize=8)
plt.tight_layout()

#%%
bb  = np.argsort(np.sum(CCts[:, :, 0], axis=0))
idx = (0, 3, 8)
for j in range(3):
    ...
    for i in range(len(idx)):
        ...
        plt.imshow(
            c[bb[:, None], bb, idx[i]] / trial_bins[1],
