...
polygons = []
ops = np.load('//allen/aind/scratch/BCI/2p-raw/BCINM_006/011124/suite2p_BCI/plane0/ops.npy', allow_pickle=True).tolist()
...
folder = r'//allen/aind/scratch/BCI/2p-raw/BCINM_006/011124/suite2p_BCI/plane0/reg_tif_chan2/'
folder_props = folder_props_fun.folder_props_fun(folder)
tifs = folder_props['siFiles']
num_tifs = len(tifs)
for fi in range(num_tifs):
    # Path to your TIFF file
    #file_path = ops['data_path'][0] + ops['tiff_list'][fi]
    
    file_path = folder + tifs[fi]
    print(file_path)
...
plt.subplot(212)
plt.imshow(np.nanmean(sta[0:180, :, :], axis=1).T, vmin=3, vmax=4.5, aspect='auto')
plt.xlim([0, 179])  # Ensure x-axis limits are consistent with the first plot
