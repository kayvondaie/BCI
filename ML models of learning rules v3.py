...
# Save as .npy file (not .npz)
npy_path = os.path.join(save_dir, "hebbian_fit_inputs2.npy")
np.save(npy_path, data_dict, allow_pickle=True)

print("Saved input data to:", npy_path)
...
npy_path = r"C:\Users\kayvon.daie\OneDrive - Allen Institute\Documents\Data\Figures 2025\Hebbian index fits\hebbian_fit_inputs2.npy"
...
#%%
# Sample a manageable number of trials (e.g., 1000)
X_sample = X_clean.sample(n=1000, random_state=0) if len(X_clean) > 1000 else X_clean

# Refit model on the sample if needed (or skip this if you already trained)
# model.fit(X_sample, y_clean.loc[X_sample.index])

# Compute SHAP interactions on the subset
explainer = shap.TreeExplainer(model)
interaction_values = explainer.shap_interaction_values(X_sample)

# Compute mean absolute interactions
mean_interactions = np.abs(interaction_values).mean(axis=0)

# Convert to labeled DataFrame
features = X_sample.columns
interaction_df = pd.DataFrame(mean_interactions, index=features, columns=features)

# Plot the interaction heatmap
plt.figure(figsize=(8, 6))
sns.heatmap(interaction_df, cmap="viridis", annot=True, fmt=".2e")
plt.title("Mean Absolute SHAP Interaction Values (Subset)")
plt.tight_layout()
plt.show()
...
# Top 3 features
top3 = ['Delta_THR', 'THR', 'RT']

# Select only those features
X_top3 = X_clean[top3]

# Subsample if dataset is too big
if len(X_top3) > 1000:
    X_sample = X_top3.sample(n=1000, random_state=0)
else:
    X_sample = X_top3.copy()

# Match y to X_sample
y_sample = y_clean.loc[X_sample.index]

# Fit model on the sampled subset (must match X_sample)
model_small = GradientBoostingRegressor(max_depth=3, n_estimators=50, random_state=0)
model_small.fit(X_sample, y_sample)
